...
def Integral_model(model):
    ...
    dnn_output_layer = model(x_input)
    integral_output = TrapezoidalIntegralLayer()([x_input, dnn_output_layer])
    model = Model(inputs=x_input, outputs=integral_output)
    model.dnn_output_layer = dnn_output_layer  # Labeling dnn_output as a layer
    return model
...
# Create a new model to access the dnn_output_layer
x_input = layers.Input(shape=(1,))
dnn_output_model = Model(inputs=x_input, outputs=amodel.dnn_output_layer)

# Generate predictions from the DNN model
dnn_predictions = dnn_output_model.predict(x_train)
