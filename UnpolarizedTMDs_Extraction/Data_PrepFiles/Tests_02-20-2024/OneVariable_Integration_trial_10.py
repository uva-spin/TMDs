...
# def Amodel(model):
#     y = model.output
#     integral = layers.Lambda(lambda x: tf.reduce_sum(x, axis=1))(y)
#     return tf.keras.Model(inputs=model.input, outputs=integral)

# def Amodel(model):
#     y = model.output
#     y = layers.Lambda(lambda x: x, name='y')(y)  # Labeling the y layer
#     integral = layers.Lambda(lambda x: tf.reduce_sum(x, axis=1))(y)
#     return tf.keras.Model(inputs=model.input, outputs=integral)
...
def Amodel(model):
    ...
    integral = layers.Lambda(simpsons_rule_integration)(y)
    return tf.keras.Model(inputs=model.input, outputs=integral)
...
# Evaluate f(x) for the range of x values
f_values = f(x_values)


# Define a new model to extract the output of the y layer
y_output_model = tf.keras.Model(inputs=amodel.input, outputs=amodel.get_layer('y').output)

# Obtain the predictions from the y layer
y_predictions = y_output_model.predict(x_train)

# Plot f(x) and the predictions from the y layer
plt.figure(figsize=(10, 6))
#plt.plot(x_values, f_values, label='f(x) = 2x + 1', linestyle='--', color='blue')  # Plot f(x)
plt.plot(x_train, y_predictions, label='DNN Model Output (y)', color='orange')  # Plot the predictions from the y layer
plt.xlabel('x')
plt.ylabel('Value')
plt.title('f(x) vs DNN Model Output (y)')
plt.legend()
plt.grid(True)
plt.show()
