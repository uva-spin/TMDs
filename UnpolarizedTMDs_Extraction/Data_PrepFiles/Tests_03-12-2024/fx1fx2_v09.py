...
def SigmaModel():
    ...
    modfx1 = create_nn_model('fx1')
    modfx2 = create_nn_model('fx2')

    modfx1_vals = modfx1([x1])
    modfx2_vals = modfx2([x2])

    product = tf.keras.layers.Multiply()([modfx1_vals,modfx2_vals])
...
def fx1fx2_product_loss(y_true, y_pred):
    fx1result_model = model.get_layer('fx1')
    fx2result_model = model.get_layer('fx2')
    #fx2_true = f(x2_values) * Sk(kk_values_loss)
    fx1_result = fx1result_model(x1Vals)
    fx2_result = fx2result_model(x2Vals)
    fx1x2 = fx1_result*fx2_result
    # fx1_result_rev = fx1result_model(concatenated_inputs_rev)
    # fx2_result_rev = fx2result_model(concatenated_inputs_rev)
    # fx2x1 = tf.Multipy()([fx1_result_rev,fx2_result_rev])
    # pdf2_loss = tf.reduce_mean(tf.square(fx1x2 - fx2x1))
...
def custom_loss(y_true, y_pred):
    return mse_loss(y_true, y_pred) + fx1fx2_product_loss(y_true, y_pred)
...
# model.compile(optimizer=tf.keras.optimizers.Adam(lr), loss=custom_loss, metrics=[mse_loss,fx1fx2_product_loss])

# history = model.fit(concatenated_inputs, Avals, epochs=200, verbose=2,callbacks=[CustomLossVerboseCallback()])
history = model.fit([df['x1'],df['x2']], Avals, epochs=300, verbose=2)
...
plt.legend()
#plt.show()
plt.savefig('f(x2x1)_f(x1x2).pdf')
