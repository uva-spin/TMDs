...
fqbar_output_vals = fq_model.predict([x2vals, pTmkvals])/(kAvg[1]-kAvg[0])
#fq_output_vals = np.sqrt(fq_output_vals)

# Generate fxSk values for comparison
fxSk_vals = fxSk(fux1[0], kAvg)  # Assuming we are comparing for the first set of fux1 and kAvg
...
plt.plot(kAvg, fqbar_output_vals, '*', label='fq output', color='red')
