...
QMvals = np.linspace(2.4,2.4,len(x1vals)) ## Here we fixed the QM dependence as Q2
...
def Apseudo(x1,x2,pT,QM):
    tempx1, tempx2, temppT, tempQM, tempA, temp_pdf_u, temp_pdf_ubar = [], [], [], [], [], [], []
    ...
    return np.array(tempx1), np.array(tempx2), np.array(temppT), np.array(tempQM), np.array(tempA), np.array(temp_pdf_u), np.array(temp_pdf_ubar)

x1Vals, x2Vals, pTVals, QMVals, Avals, pdfUvals, pdfUbarvals = Apseudo(x1vals,x2vals,pTvals,QMvals)

df = pd.DataFrame({'x1': x1Vals, 'x2': x2Vals, 'pT': pTVals, 'QM': QMVals, 'A': Avals, 'fu': pdfUvals, 'fubar': pdfUbarvals})
...
def build_model(hp):
    ...
    for i in range(hp.Int('num_layers', 1, 7)):
        model.add(layers.Dense(
            ...
            activation=hp.Choice('activation_' + str(i), ['relu', 'relu6', 'tanh', 'selu', 'sigmoid', 'softmax'])
