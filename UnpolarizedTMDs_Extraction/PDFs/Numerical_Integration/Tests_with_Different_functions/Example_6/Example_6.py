...
## (1) MSE and we implemented the loss function for the pdfs
...
#     tempx1, tempx2, temppT, tempA = [], [], [], []np.exp(-k**2/4)
...
# def create_nn_model(name, hidden_layers=Hidden_Layers, width=Nodes_per_HL, activation='relu6'):
#     inp = tf.keras.Input(shape=(2))
#     x = tf.keras.layers.Dense(width, activation='tanh')(inp)
#     for i in range(hidden_layers-1):
#         x = tf.keras.layers.Dense(width, activation=activation)(x)
#     nnout = tf.keras.layers.Dense(1, activation='sigmoid')(x)
#     mod = tf.keras.Model(inp, nnout, name=name)
#     return mod
...
def createModel_DY():
    ...
    pdfs_input_fu = tf.keras.layers.Concatenate()([x1, x1*0])
    pdfs_input_fub = tf.keras.layers.Concatenate()([x2, x2*0])
    nnu_pdf = modnnu(pdfs_input_fu)
    nnubar_pdf = modnnu(pdfs_input_fub)
...
def prod1(x,k):
    ...
    for i in range(len(x)):
        temp = fu_val(x[i])*Sk(k[i])
        temp_prod.append(temp)
    return np.array(temp_prod)

def prod2(x,k):
    temp_prod =[]
    for i in range(len(x)):
        temp = fubar_val(x[i])*Sk(k[i])
        temp_prod.append(temp)
    return np.array(temp_prod)

true_values_1 = prod1(x1vals,Kvals)
true_values_pdf1 = fu   
true_values_2 = prod2(x2vals,pT_k_vals)
true_values_pdf2 = fubar 
...
plt.plot(x1vals, true_values_1, label='$f_u(x)S(k)$')
plt.plot(x2vals, true_values_2, label='$f_{\\bar{u}}(x)S(p_T-k)$')
plt.title('NNPDF4 fu and fubar')
plt.xlabel('x')
plt.ylabel('f')
plt.legend()
plt.grid(True)
#plt.show()
plt.savefig('TMDs_true_functions.pdf')
...
plt.plot(x1vals, true_values_pdf1, label='True fu', linestyle='--')
...
plt.plot(x2vals, true_values_pdf2, label='True fubar', linestyle='--')
