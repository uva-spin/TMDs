...
def createModel_DY():
    ...
    modnnu = create_nn_model('nnu')(nnu_input)
    modnnubar = create_nn_model('nnubar')(nnubar_input)

    product_list=tf.multiply(modnnu, modnnubar)

    # Summing over all k values using tf.reduce_sum
    tmd_product_sum = product_list

    return tf.keras.Model([x1, x2, qT], [tmd_product_sum, modnnu, modnnubar])
