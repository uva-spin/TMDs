...
def fx1kx2k(x1,x2,pT,k):
    return fu_val(x1)*Sk(k)*fubar_val(x2)*Sk(pT-k)


def Apseudo(x1,x2,pT):
    tempx1, tempx2, temppT, tempA = [], [], [], []
    kk = np.linspace(0.1,2.0,len(x1vals))
    for i in range(len(x1)):
        tempx1.append(x1[i])
        tempx2.append(x2[i])
        temppT.append(pT[i])
        tempfx1kfx2k = fx1kx2k(x1[i],x2[i],pT[i],kk[i])
...
def createModel_DY():
    ...
    modnnu_eval = modnnu(nnu_input)
    modnnubar_eval = modnnubar(nnubar_input)

    product_list=tf.multiply(modnnu_eval, modnnubar_eval)

    # Summing over all k values using tf.reduce_sum
    tmd_product_sum = product_list

    return tf.keras.Model([x1, x2, qT], [tmd_product_sum, modnnu_eval, modnnubar_eval])
