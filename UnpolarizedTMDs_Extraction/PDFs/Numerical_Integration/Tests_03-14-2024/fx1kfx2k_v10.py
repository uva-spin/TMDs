...
# class Tmd1SumLayer(tf.keras.layers.Layer):
#     def __init__(self, **kwargs):
#         super(Tmd1SumLayer, self).__init__(**kwargs)

#     def call(self, inputs):
#         tmd1_sum = tf.reduce_sum(inputs, axis=0)
#         return tmd1_sum

# class Tmd2SumLayer(tf.keras.layers.Layer):
#     def __init__(self, **kwargs):
#         super(Tmd2SumLayer, self).__init__(**kwargs)

#     def call(self, inputs):
#         tmd2_sum = tf.reduce_sum(inputs, axis=0)
#         return tmd2_sum

# def custom_loss(y_true, y_pred):
#     tmd1_sum = model.get_layer('tmd1_sum').output
#     tmd2_sum = model.get_layer('tmd2_sum').output

#     # Compute the loss using symbolic tensors
#     pdf1_loss = tf.reduce_mean(tf.square(y_true - tmd1_sum))
#     pdf2_loss = tf.reduce_mean(tf.square(y_true - tmd2_sum))
#     mse_loss = tf.reduce_mean(tf.square(y_true - y_pred))
    
#     # Combine the losses
#     total_loss = mse_loss + pdf1_loss + pdf2_loss
#     return total_loss
...
#     # Summing over all k values using custom layers
#     tmd1_sum = Tmd1SumLayer(name='tmd1_sum')(tf.stack(tmd1_list))
#     tmd2_sum = Tmd2SumLayer(name='tmd2_sum')(tf.stack(tmd2_list))
...
#     # k_values = tf.linspace(0.1, 100, 100)  # Generate k values
#     k_values = tf.linspace(0.0, 2.0, 100)
#     dk = k_values[1] - k_values[0]
...
def custom_loss(y_true, y_pred):
    # Access the tmd1_sum and tmd2_sum tensors directly from the model's output
    tmd1_sum = model.layers[-1](model.input)
