...
# test_tmd1_sum = model.layers[-3].output
# print(test_tmd1_sum)

# def pdf1_loss_val(y_true, y_pred):
#     concatenated_inputs_1 = np.column_stack((x1Vals, x2Vals, pTVals))
#     fx1_result = model.layers[-3](concatenated_inputs_1)
...
# def pdf1_loss_val(y_true, y_pred):
#     concatenated_inputs_1 = np.column_stack((x1vals, x2vals, pTvals))
#     fx1_result = tf.reshape(model.layers[-3](concatenated_inputs_1), (-1, 1))
#     fx1_true = (f(x1vals) * Sk(Kvals)).reshape(-1, 1)
#     pdf1_loss = fx1_true - fx1_result
#     return pdf1_loss
...
# def pdf1_loss_val(y_true, y_pred):
#     concatenated_inputs_1 = np.column_stack((x1vals, x2vals, pTvals))
#     fx1_result = model.layers[-2](concatenated_inputs_1)
#     fx1_true = (f(x1vals) * Sk(Kvals)).reshape(-1, 1)
#     print("fx1_result:", fx1_result)
#     print("fx1_true:", fx1_true)
#     pdf1_loss = fx1_true - fx1_result
#     return pdf1_loss

## Worked but gave constant value
# def pdf1_loss_val(y_true, y_pred):
#     concatenated_inputs_1 = np.column_stack((x1vals, x2vals, pTvals))
#     fx1_result = model.layers[-3](concatenated_inputs_1)
#     fx1_true = (f(x1vals) * Sk(Kvals)).reshape(-1, 1)
...
def pdf1_loss_val(y_true, y_pred):
    ...
    fx1_result = model.layers[-3](concatenated_inputs_1)
    fx1_true = (f(x1vals) * Sk(Kvals)).reshape(-1, 1)
    fx1_result_selected = fx1_result[:, 0]  # Selecting the first column of fx1_result
    pdf1_loss = tf.reduce_mean(tf.square(fx1_true - fx1_result_selected))
    return pdf1_loss
    # pdf1_loss = fx1_true - fx1_result
    # return pdf1_loss
...
def custom_loss(y_true, y_pred):
    return mse_loss(y_true, y_pred) + tf.cast(pdf1_loss_val(y_true, y_pred), dtype=tf.float32) + pdf2_loss_val(y_true, y_pred)

# def custom_loss(y_true, y_pred):
#     return mse_loss(y_true, y_pred) + pdf1_loss_val(y_true, y_pred) + pdf2_loss_val(y_true, y_pred)

# + tf.cast(pdf1_loss_val(y_true, y_pred), dtype=tf.float32)
# def custom_loss(y_true, y_pred):
#     return mse_loss(y_true, y_pred) + pdf2_loss_val(y_true, y_pred)
