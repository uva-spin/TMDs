...
def DNN_model(width=100, L1_reg=10**(-12), activation='relu'):
    ...
    #fx1fx2 = tf.keras.layers.Dense(2, activation=activation, kernel_initializer=initializer, activity_regularizer=tf.keras.regularizers.L1(L1_reg), name='fx1fx2')(x2)
    ...
    mod = tf.keras.Model(inputs=inp, outputs=[product,x1,fx2])
    return mod
...
history = model.fit(concatenated_inputs, Avals, epochs=20, verbose=2)

# output= model.predict(concatenated_inputs)
# predvals = np.array(output)
# truevals = np.array(Avals)
# print(predvals-truevals)
