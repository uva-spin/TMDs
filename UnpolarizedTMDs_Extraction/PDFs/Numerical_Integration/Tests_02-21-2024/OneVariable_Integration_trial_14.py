...
# Define the loss function using the integral output
def custom_loss(y_true, y_pred):
    integral_pred = tf.reduce_sum(y_pred)
    integral_true = tf.reduce_sum(y_true)
    return tf.abs(integral_true - integral_pred)

# Compile the model
model.compile(optimizer='adam', loss=custom_loss)

# Train the model
model.fit(df['x'].values, df['A'].values, epochs=100, batch_size=32)

# Evaluate the model
loss = model.evaluate(df['x'].values, df['A'].values)
...
# Plot results
plt.scatter(df['x'], df['A'], label='Pseudodata')
plt.plot(df['x'], predictions, color='red', label='Model Prediction')
plt.xlabel('x')
plt.ylabel('A')
plt.legend()
plt.show()
