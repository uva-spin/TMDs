...
create_folders('Evaluations_TMDs_increasing_k')
...
def Generate_Comparison_Plots(df, num_replicas, output_name):
    ...
    plt.plot(Kvals, true_values_1, label='True nnu', linestyle='--')
    plt.plot(Kvals, tempfu_mean, 'r', label='Predicted nnu')
    plt.fill_between(Kvals, (tempfu_mean-tempfu_err).flatten(), (tempfu_mean+tempfu_err).flatten(), facecolor='r', alpha=0.3)
    ...
    plt.grid(True)
    #plt.show()
    plt.savefig('Evaluations_TMDs_increasing_k/True_Pred_fxk_q_vs_k'+str(output_name)+'.pdf')
    plt.close()

    plt.figure(2, figsize=(10, 6))
    plt.plot(Kvals, true_values_2, label='True nnubar', linestyle='--')
    plt.plot(Kvals, tempfubar_mean, 'r', label='Predicted nnubar')
    plt.fill_between(Kvals, (tempfubar_mean-tempfubar_err).flatten(), (tempfubar_mean+tempfubar_err).flatten(), facecolor='r', alpha=0.3)
    ...
    plt.grid(True)
    #plt.show()
    plt.savefig('Evaluations_TMDs_increasing_k/True_Pred_fxk_qbar_vs_k'+str(output_name)+'.pdf')
    plt.close()
