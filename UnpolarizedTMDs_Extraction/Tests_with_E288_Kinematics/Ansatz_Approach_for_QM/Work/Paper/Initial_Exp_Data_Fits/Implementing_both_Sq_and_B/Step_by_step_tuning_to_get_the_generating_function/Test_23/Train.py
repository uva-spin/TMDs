...
def GenerateReplicaData(df):
    ...
    qTcorr = 1 + ((qT_values*qT_values)/(QM_values*QM_values))
    SB = ReplicaA *SqrtS * SqrtS * qTcorr / (hc_factor * factor * PDFs * QM_integral)
...
def compute_A(model, x1, x2, qT, QM, SqrtS):
    ...
    QM_integral_temp = compute_QM_integrals(QM)
    A_pred = SB_DNN_mean * factor_temp * PDFs_temp * hc_factor * QM_integral_temp /(SqrtS*SqrtS*qTcorr)
    return A_pred
