...
# def train_weighted_loss(y_true, y_pred, weights=None):
#     """
#     Weighted loss function that properly handles batching.
#     """
...
#         # Ensure weights are properly shaped for the current batch
#         weights = tf.convert_to_tensor(weights, dtype=tf.float32)
#         squared_error = squared_error * weights
        
#     return tf.reduce_mean(squared_error)

# # Create a custom weighted loss function for model.compile
# def create_weighted_loss(weights):
#     def weighted_loss(y_true, y_pred):
#         return train_weighted_loss(y_true, y_pred, weights)
...
#         model.compile(optimizer=optimizer, loss='mse')  # Use standard MSE loss
        
#         # Train for this stage, using sample_weight parameter
#         history = model.fit(
#             x=[X_train[0], X_train[1]],
#             y=y_train,
#             validation_data=([X_val[0], X_val[1]], y_val, w_val),
#             epochs=epochs_per_stage,
#             batch_size=batch_size,
#             sample_weight=w_train,  # Pass weights directly
#             callbacks=[modify_LR],
#             verbose=1
#         )
...
#     """
#     Weighted loss function for use with model.compile.
#     Note: This doesn't take weights directly - they'll be applied by Keras.
#     """
#     # Calculate squared error
#     squared_error = tf.square(y_pred - y_true)
#     # Return mean (Keras will multiply by sample_weight)
#     return tf.reduce_mean(squared_error)
