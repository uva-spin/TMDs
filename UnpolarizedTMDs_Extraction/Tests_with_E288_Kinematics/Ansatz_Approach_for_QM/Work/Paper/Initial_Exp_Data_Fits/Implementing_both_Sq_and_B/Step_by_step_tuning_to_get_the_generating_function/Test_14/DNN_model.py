...
#     inp = tf.keras.Input(shape=(1,))
#     initializer = tf.keras.initializers.RandomUniform(minval=-0.1,maxval=0.1,seed=None)
#     x1 = tf.keras.layers.Dense(200, activation='relu', kernel_initializer = initializer)(inp)
#     x2 = tf.keras.layers.Dense(100, activation='tanh', kernel_initializer = initializer)(x1)
#     x3 = tf.keras.layers.Dense(50, activation='relu', kernel_initializer = initializer)(x2)
...
def DNNS(name):
    ...
    for i in range(0,2):
        x = tf.keras.layers.Dense(500, activation='tanh', kernel_initializer = initializer)(x)
    out = tf.keras.layers.Dense(1, activation='softplus', kernel_initializer = initializer)(x)
    return tf.keras.Model(inp, out, name=name)
