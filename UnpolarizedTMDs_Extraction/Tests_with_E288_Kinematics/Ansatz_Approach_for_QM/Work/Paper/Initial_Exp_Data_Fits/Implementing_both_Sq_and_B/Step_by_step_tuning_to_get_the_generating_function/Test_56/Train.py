...
#     xF = tf.keras.Input(shape=(1,), name='xF')

#     SModel = DNNS('SqT')
#     BModel = DNNB('BQM')
    
#     # Concatenate qT and QM inputs
#     concatenated = tf.keras.layers.Concatenate()([qT, xF])
    
#     Sq = SModel(concatenated)
#     BQM = BModel(QM)
    
#     SB = tf.keras.layers.Multiply()([Sq, BQM])
#     return tf.keras.Model([qT, QM, xF], SB)
...
def SB_model():
    ...
    # Concatenate qT and xF for the DNNS model
    concatenated = tf.keras.layers.Concatenate()([qT, xF])
    
    Sq = SModel(concatenated)  # Pass the concatenated tensor to DNNS
    BQM = BModel(QM)
    
    SB = tf.keras.layers.Multiply()([Sq, BQM])
    return tf.keras.Model([qT, QM, xF], SB)  # Note: three inputs now
...
E288_200 = add_weight_column(E288_200_initial, 200,'n')
E288_300 = add_weight_column(E288_300_initial, 300,'n')
E288_400 = add_weight_column(E288_400_initial, 400,'n')
...
def compute_A(model, x1, x2, qT, QM, xF):
    # Get Predictions from All Models
    SB_DNN_mean = model.predict([qT,QM,xF], verbose=0).flatten()
...
def prep_data_for_plots(model,df):
    ...
    A_pred = compute_A(model, x1, x2, qT, QM, xF)
...
def gen_SB_plots(model, df,replica_id):
    ...
    QM_test = np.linspace(df['QM'].min(), df['QM'].max(), 200)
    qT_test = np.linspace(df['qT'].min(), df['qT'].max(), 200)
    xF_test = np.zeros_like(qT_test)  # Add a placeholder for xF
    
    # Now pass all 3 inputs
    dnnQ_contributions = model.predict([qT_test,QM_test, xF_test], verbose=0).flatten()
...
def train_progressively(model, X_train, y_train, X_val, y_val, w_train, w_val,
    ...
    for stage in range(1, depth + 1):
        ...
        # Train for this stage, using sample_weight parameter
        history = model.fit(
            x=[X_train[0], X_train[1], X_train[2]],  # List of all 3 input arrays
            y=y_train,                    # Target values
            validation_data=([X_val[0], X_val[1], X_val[2]], y_val, w_val),  # Include all 3 inputs
            epochs=epochs_per_stage,
            batch_size=batch_size,
...
def replica_model(i):
    ...
    trained_model, all_history = train_progressively(
        dnnSB,
        X_train=[qT_train, QM_train, xF_train],
        y_train=SB_train,
        X_val=[qT_test, QM_test, xF_test],
        y_val=SB_test,
