...
# # Define Weighted Loss Function based on SqrtS
...
# # Create a function that returns a loss function with specific weights
# def create_weighted_loss_function(SqrtS_values):
#     # Convert SqrtS values to weights
#     # You can customize this function to create weights based on your preference
#     # For example, giving higher weights to higher SqrtS values
#     weights = tf.cast(SqrtS_values / tf.reduce_mean(SqrtS_values), dtype=tf.float32)
...
#     # Make sure weights has the same shape as squared_error
#     # Reshape weights to match squared_error if needed
#     weights = tf.reshape(weights, [-1, 1])  # Reshape to column vector
    
#     # Make sure they have the same datatype
...
# def create_weighted_loss_function(SqrtS_values):
#     # Convert SqrtS values to weights
#     SqrtS_values = tf.convert_to_tensor(SqrtS_values, dtype=tf.float32)
#     weights = SqrtS_values / tf.reduce_mean(SqrtS_values)
...
#     def __init__(self, x_val, y_val, sqrt_s_val):
#         super().__init__()
#         self.x_val = x_val
#         self.y_val = y_val
#         self.sqrt_s_val = sqrt_s_val
#         # Create weighted loss function for validation
#         self.weighted_loss_fn = create_weighted_loss_function(self.sqrt_s_val)
        
#     def on_epoch_end(self, epoch, logs=None):
#         logs = logs or {}
#         val_pred = self.model.predict(self.x_val, verbose=0)
#         # Make sure tensors have correct shape and type
#         y_val_tensor = tf.convert_to_tensor(self.y_val, dtype=tf.float32)
#         val_pred_tensor = tf.convert_to_tensor(val_pred, dtype=tf.float32)
        
#         val_weighted_loss = self.weighted_loss_fn(y_val_tensor, val_pred_tensor)
#         logs['val_loss'] = val_weighted_loss
#         print(f"\nEpoch {epoch+1}: val_weighted_loss = {val_weighted_loss:.4f}")


# Define Weighted Loss Function based on SqrtS
def custom_weighted_loss(y_true, y_pred, sqrt_s):
    # Convert inputs to tensors with proper type
    y_true = tf.cast(y_true, tf.float32)
    y_pred = tf.cast(y_pred, tf.float32)
    sqrt_s = tf.cast(sqrt_s, tf.float32)
    
    # Calculate weights based on SqrtS
    mean_sqrt_s = tf.reduce_mean(sqrt_s)
    weights = sqrt_s / mean_sqrt_s
    
    # Calculate squared error
    squared_error = tf.square(y_pred - y_true)
    
    # Apply weights - ensure they're the same shape
    weighted_squared_error = squared_error * weights
    
    # Return mean of weighted squared errors
    return tf.reduce_mean(weighted_squared_error)

# Create a custom callback for weighted validation loss
class WeightedValidationLoss(tf.keras.callbacks.Callback):
    def __init__(self, validation_data, sqrt_s_test):
        super(WeightedValidationLoss, self).__init__()
        self.validation_data = validation_data
        self.sqrt_s_test = sqrt_s_test
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        val_loss = custom_weighted_loss(y_test, y_pred, self.sqrt_s_test)
...
# Modify the train function to use weighted loss
def replica_model(i):
    ...
    # # Create weighted loss functions for training and validation
    # train_weighted_loss = create_weighted_loss_function(SqrtS_train)
    # test_weighted_loss = create_weighted_loss_function(SqrtS_test)
    ...
    def train_weighted_loss(y_true, y_pred):
        return custom_weighted_loss(y_true, y_pred, SqrtS_train)
    ...
    # # Use the weighted loss function for training
    # dnnSB.compile(optimizer=optimizer, loss=train_weighted_loss)
    
    # # Create a custom callback to evaluate validation loss with weighted loss
    # class WeightedValidationLoss(tf.keras.callbacks.Callback):
    #     def on_epoch_end(self, epoch, logs=None):
    #         val_pred = self.model.predict([qT_test, QM_test], verbose=0)
    #         val_weighted_loss = test_weighted_loss(SB_test, val_pred).numpy()
    #         logs['val_loss'] = val_weighted_loss
    #         print(f"\nEpoch {epoch+1}: val_weighted_loss = {val_weighted_loss:.4f}")
    
    # # Combine the callbacks
    # callbacks = [modify_LR, WeightedValidationLoss()]
    ...
    # # In your replica_model function:
    # val_loss_callback = WeightedValidationLoss(
    #     x_val=[qT_test, QM_test],
    #     y_val=SB_test,
    #     sqrt_s_val=SqrtS_test
    # )
    ...
    plt.title(f'Model {i} Loss over Epochs (SqrtS-Weighted)')
    plt.xlabel('Epochs')
    plt.ylabel('Weighted Loss')
    plt.legend()
    plt.grid(True)
    loss_plot_path = os.path.join(loss_plot_folder, f'weighted_loss_plot_model_{i}.pdf')
