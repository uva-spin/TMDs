...
# Missing import - assuming this would be imported from a local module
# If functions_and_constants.py doesn't exist, we need to define these constants here
# For now, I'll define them here
# Constants definition
eu2 = 4/9  # Up quark charge squared
ed2 = 1/9  # Down quark charge squared
es2 = 1/9  # Strange quark charge squared
factor = 1.0  # Generic factor used in calculations
Num_Replicas = 10  # Number of replicas to generate
...
# Define WeightedValidationLoss class that was referenced but not defined
class WeightedValidationLoss(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # Define custom_weighted_loss here to ensure it's accessible
...
E288_200 = add_weight_column(E288_200_initial, 100000, 7.0, 'y')
E288_300 = add_weight_column(E288_300_initial, 1, 4.0, 'n')
E288_400 = add_weight_column(E288_400_initial, 100000, 7.7, 'y')
...
def gen_SB_plots(model, df,replica_id):
    ...
    pairs = np.vstack([np.repeat(qT_test_reshaped[0], len(QM_test_reshaped)), QM_test_reshaped.flatten()]).T
    
    # Get Model Predictions for SB(qT,QM)
    dnnQ_contributions = model.predict([pairs[:, 0].reshape(-1, 1), pairs[:, 1].reshape(-1, 1)], verbose=0).flatten()
    
    # Plot Model Predictions
    plt.figure(figsize=(10, 6))
    plt.plot(QM_test, dnnQ_contributions, label='DNNQ Model Mean', linestyle='-', color='red')
    plt.xlabel(r'$Q_M$', fontsize=14)
    plt.ylabel(r'$SB(q_T, Q_M)$', fontsize=14)
    plt.title('SB Model Predictions', fontsize=16)
...
def split_data(X, y, split=0.1):
    ...
    # Create copy of dataframes to avoid modifying originals
    X_copy = X.copy()
    y_copy = y.copy()
    
    test_X = X_copy.iloc[temp].reset_index(drop=True)
    train_X = X_copy.drop(temp).reset_index(drop=True)
    
    test_y = y_copy.iloc[temp].reset_index(drop=True)
    train_y = y_copy.drop(temp).reset_index(drop=True)
...
#     """
#     # Extract weights for training and validation
#     w_train = X_train[1].get('weight', np.ones(len(y_train)))
...
#     # Use correctly implemented split_data function
#     prep_features = replica_data.drop(['SB_calc'], axis=1)
#     prep_SB = replica_data['SB_calc']
#     train_X, test_X, train_SB, test_SB = split_data(prep_features, prep_SB)
    
#     qT_train = np.array(train_X['qT'])
#     QM_train = np.array(train_X['QM'])
#     SB_train = np.array(train_SB)  # Fixed: using train_SB instead of train_X['SB_calc']
...
#     SB_test = np.array(test_SB)  # Fixed: using test_SB instead of test_X['SB_calc']
#     SqrtS_test = np.array(test_X['SqrtS'])
#     weights_test = np.array(test_X['weight'])
    
#     # Create input dictionary with weights for training
#     train_input = [qT_train, {'QM': QM_train, 'weight': weights_train}]  # Fixed: proper input format
#     test_input = [qT_test, {'QM': QM_test, 'weight': weights_test}]  # Fixed: proper input format
...
#         dnnSB,
#         X_train=train_input,  # Fixed: using proper input format
#         y_train=SB_train,
#         X_val=test_input,  # Fixed: using proper input format
#         y_val=SB_test,
#         freeze_prev=True,
...
def train_progressively(model, X_train, y_train, X_val, y_val,
    ...
    if isinstance(X_train, list) and len(X_train) > 1 and isinstance(X_train[1], dict) and 'weight' in X_train[1]:
        w_train = X_train[1]['weight']
    else:
    ...
    for stage in range(1, depth + 1):
        ...
        # Extract actual input tensors from X_train and X_val based on the model's expected input shape
        if isinstance(X_train, list):
            if isinstance(X_train[1], dict):
                # If second element is a dict, extract QM values
                train_inputs = [X_train[0], X_train[1]['QM']]
            else:
        ...
        if isinstance(X_val, list):
            if isinstance(X_val[1], dict):
                # If second element is a dict, extract QM values
                val_inputs = [X_val[0], X_val[1]['QM']]
            else:
                val_inputs = X_val
        else:
            val_inputs = X_val
...
def replica_model(i):
    ...
    qT_train = np.array(train_X['qT'])
    QM_train = np.array(train_X['QM'])
    SB_train = np.array(train_SB)
    weights_train = np.array(train_X['weight'])
    
    qT_test = np.array(test_X['qT'])
    QM_test = np.array(test_X['QM'])
    SB_test = np.array(test_SB)
    weights_test = np.array(test_X['weight'])
    
    # These arrays are what the model actually expects
    train_inputs = [qT_train, QM_train]
    test_inputs = [qT_test, QM_test]
    
    # Create the dictionary structure to pass weights without affecting the model inputs
    train_input_with_weights = [qT_train, {'QM': QM_train, 'weight': weights_train}]
    test_input_with_weights = [qT_test, {'QM': QM_test, 'weight': weights_test}]
    ...
    trained_model, all_history = train_progressively(
        dnnSB,
        X_train=train_input_with_weights,
        y_train=SB_train,
        X_val=test_input_with_weights,
        y_val=SB_test,
        freeze_prev=True,
