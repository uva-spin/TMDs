...
#     Sq = SModel(qT)
#     BQM = BModel(QM)
#     fDNNqQ = fDNNModel([qT,QM])

#     SB = tf.keras.layers.Multiply()([Sq, BQM, fDNNqQ])
...
def SB_model():
    ...
    concatenated = tf.keras.layers.Concatenate()([qT, QM])
    
    Sq = SModel(qT)
    BQM = BModel(QM)
    fDNNqQ = fDNNModel(concatenated)  # Pass the concatenated input
