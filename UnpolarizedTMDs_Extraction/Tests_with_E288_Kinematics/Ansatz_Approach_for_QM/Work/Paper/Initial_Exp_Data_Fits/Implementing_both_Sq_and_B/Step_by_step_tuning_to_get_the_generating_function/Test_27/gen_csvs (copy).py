...
# # Define the same weighted loss function we used during training
# def custom_weighted_loss(y_true, y_pred):
#     # Since we don't have SqrtS during loading, we'll use a placeholder version
#     # that simply returns MSE - this is just for loading the model
...
#     custom_objects={'custom_loss': custom_weighted_loss}
# ) for f in model_files]


# Define the custom weighted loss function as it was during training
def custom_weighted_loss(y_true, y_pred, sqrt_s=None):
    y_true = tf.cast(y_true, tf.float32)
    y_pred = tf.cast(y_pred, tf.float32)
    
    # If sqrt_s is provided (during training), use weights
    ...
    # For model loading (when sqrt_s not available), fall back to MSE
    else:
        return tf.reduce_mean(tf.square(y_pred - y_true))
