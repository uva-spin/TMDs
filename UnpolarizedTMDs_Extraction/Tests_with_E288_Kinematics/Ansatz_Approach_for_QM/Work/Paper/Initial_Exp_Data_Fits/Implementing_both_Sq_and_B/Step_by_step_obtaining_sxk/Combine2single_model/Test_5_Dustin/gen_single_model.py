...
#     all_model_weights = [model.get_weights() for model in models_list]
    
#     # Calculate the average weights across all models
#     avg_weights = []
#     for i in range(len(all_model_weights[0])):
#         # Stack the i-th layer weights from all models
#         layer_weights = [model_weight[i] for model_weight in all_model_weights]
...
# def average_models(models_list): 
#     if not models_list: raise ValueError("No models provided") 
#     # Clone architecture 
#     avg_model = tf.keras.models.clone_model(models_list[0]) 
#     avg_model.build(models_list[0].input_shape) # Ensure weights are initialized 
#     # Extract weights 
#     all_weights = [model.get_weights() for model in models_list] 
#     # Elementwise average 
#     avg_weights = [ np.mean(np.stack([weights[i] for weights in all_weights]), axis=0) for i in range(len(all_weights[0])) ] 
...
#     fresh_model = SB_model()
#     weights_list = [model.get_weights() for model in models_list]
...
def find_closest_model_to_mean(models_list): 
    if not models_list: 
        raise ValueError("No models provided") 
    ...
    mean_weights = [ np.mean(np.stack([weights[i] for weights in weights_list]), axis=0) for i in range(len(weights_list[0])) ] 
    
    # Define a simple distance metric (e.g., L2 norm of all layers) 
    def distance_to_mean(weights): 
        return sum(np.linalg.norm(w - mw) for w, mw in zip(weights, mean_weights)) 
    
    # Compute distance for each model 
    distances = [distance_to_mean(weights) for weights in weights_list] 
    
    # Find the index of the model closest to the mean 
    closest_index = int(np.argmin(distances)) 
    
    return models_list[closest_index]
