...
for xF in xF_values:
    ...
    for x1 in x1_values:
        ...
        for qT in qT_values:
            valid_combinations.append((qT, x1, x2, xF))
...
QM_array = np.full(len(valid_combinations), QM, dtype=np.float32)
...
for batch_idx in range(total_batches):
    ...
    for i in range(batch_size):
        x1, x2 = x1_batch[i, 0], x2_batch[i, 0]
...
csv_filename = f'{results_folder}/grid_results_xF{NUM_XF_VALUES}_x1{NUM_X1_VALUES}_qT{NUM_QT_VALUES}.csv'
