...
def pdf_prep(x,c0,c1,c2,c3,c4,c5):
    return np.array(c0*(x**c1)*((1-x)**c2)*np.exp(c3*x)*((1 + np.exp(c4)*x)**c5))
...
def pdf(flavor, x):
    if(flavor==1):
        ...
        c4 = 0.965
        c5 = 3.000
        temp_pdf = pdf_prep(x,c0,c1,c2,c3,c4,c5)
    if(flavor==2):
        c0 = 1
        c1 = 0.630
        c2 = 2.934
...
def GenerateReplicaData(df):
    ...
    # Compute PDFs
    f_u_x1 = pdf(2, x1_values)
    f_ubar_x2 = pdf(-2, x2_values)
    f_u_x2 = pdf(2, x2_values)
    f_ubar_x1 = pdf(-2, x1_values)
    f_d_x1 = pdf(1, x1_values)
    f_dbar_x2 = pdf(-1, x2_values)
    f_d_x2 = pdf(1, x2_values)
    f_dbar_x1 = pdf(-1, x1_values)
    f_s_x1 = pdf(3, x1_values)
    f_sbar_x2 = pdf(-3, x2_values)
    f_s_x2 = pdf(3, x2_values)
    f_sbar_x1 = pdf(-3, x1_values)

    PDFs = (eu2*(f_u_x1) * (f_ubar_x2) + 
            eu2*(f_u_x2) * (f_ubar_x1) + 
...
def compute_A(model, x1, x2, qT, QM, SqrtS):
    ...
    ux1ubarx2_temp = eu2*(f_u_x1)*(f_ubar_x2)
    ubarx1ux2_temp = eu2*(f_u_x2)*(f_ubar_x1)
    dx1dbarx2_temp = ed2*(f_d_x1)*(f_dbar_x2)
    dbarx1dx2_temp = ed2*(f_d_x2)*(f_dbar_x1)
    sx1sbarx2_temp = es2*(f_s_x1)*(f_sbar_x2)
    sbarx1sx2_temp = es2*(f_s_x2)*(f_sbar_x1)
    PDFs_temp = ux1ubarx2_temp + ubarx1ux2_temp + dx1dbarx2_temp + dbarx1dx2_temp + sx1sbarx2_temp + sbarx1sx2_temp
