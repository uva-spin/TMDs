...
# loss_plot_folder = f'Results_with_qTmax_{qTmax}/Loss_Plots'
...
def generate_replica_A_subplots(df,rep_num):
    ...
    with PdfPages(f"{replica_data_folder}/Replica_{rep_num}_Comparison_A_Subplots.pdf") as pdf:
        ...
        for i, QM_val in enumerate(unique_QM):
            ...
            axes[i].set_ylabel(r'$E\frac{d\sigma}{d^3q}$')
            axes[i].legend()
            axes[i].grid(True)
        plt.tight_layout()
        pdf.savefig(fig)
...
def gen_B_plots(model, df,replica_id):
    ...
    plt.ylabel(r'$\mathcal{B}_{DNNQ}(Q_M)$', fontsize=14)
