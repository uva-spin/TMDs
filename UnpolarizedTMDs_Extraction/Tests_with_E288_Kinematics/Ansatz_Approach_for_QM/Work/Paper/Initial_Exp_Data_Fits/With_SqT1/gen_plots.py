...
Btrue = np.array(functions_results['B_true'])
Bmean = np.array(functions_results['B_calc_mean'])
Bstd = np.array(functions_results['B_calc_std'])

Sk = np.array(functions_results['Sk'])

k = np.array(functions_results['k'])
qT = np.array(functions_results['qT'])
QM = np.array(functions_results['QM'])

B2true = np.array(functions_results['B2_true'])
B2mean = np.array(functions_results['B2_calc_mean'])
B2std = np.array(functions_results['B2_calc_std'])


SqT = np.array(functions_results['SqT'])


SB2true = np.array(functions_results['SB2true'])
SB2mean = np.array(functions_results['SB2mean'])
SB2std = np.array(functions_results['SB2std'])

SkBQMtrue = np.array(functions_results['SkBQMtrue'])
SkBQMmean = np.array(functions_results['SkBQMmean'])
SkBQMstd = np.array(functions_results['SkBQMstd'])
...
plt.plot(qT, SqT, label=r'$\mathcal{S}(q_T)$', linestyle='-', color='blue')
...
#plt.plot(k, SkBQMtrue, label=r'$S(k_{\perp})\mathcal{B}(Q_M)$ True', linestyle='--', color='blue')
plt.plot(k, SkBQMmean, label='$S(k_{\perp})\mathcal{B}(Q_M)$ DNN model (mean)', linestyle='-', color='red')
plt.fill_between(k, SkBQMmean - SkBQMstd, SkBQMmean + SkBQMstd, color='red', alpha=0.2, label='$S(k_{\perp})\mathcal{B}(Q_M)$ DNN model (std)')
plt.xlabel(r'$k_{\perp}$', fontsize=14)
plt.ylabel(r'$S(k_{\perp})\mathcal{B}(Q_M)$', fontsize=14)
plt.title('$S(k_{\perp})\mathcal{B}(Q_M)$ vs $k_{\perp}$', fontsize=16)
plt.legend(fontsize=12)
plt.grid(True)
plt.savefig(f"{results_folder}/SkBQM_plot.pdf")
plt.close()
