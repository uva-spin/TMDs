...
class CrossOverEarlyStopping(tf.keras.callbacks.Callback):
    def __init__(self, patience=3):
        super(CrossOverEarlyStopping, self).__init__()
        self.patience = patience
        self.wait = 0
        self.crossed = False  # Start monitoring only after val_loss < train_loss once
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if train_loss is not None and val_loss is not None:
            # Start monitoring only after val_loss crosses under train_loss
            if not self.crossed:
                if val_loss < train_loss:
                    self.crossed = True
                    print(f"Validation loss crossed under training loss at epoch {epoch+1}. Monitoring begins.")
                # Else: do nothing until crossover
            else:
                if val_loss > train_loss:
                    self.wait += 1
                    print(f"Epoch {epoch+1}: val_loss ({val_loss:.4f}) > train_loss ({train_loss:.4f}) — wait = {self.wait}/{self.patience}")
                    if self.wait >= self.patience:
                        print(f"\nEarly stopping at epoch {epoch+1}: val_loss remained above loss for {self.patience} consecutive epochs after crossover.")
                        self.model.stop_training = True
                else:
                    self.wait = 0  # Reset if val_loss is back below loss
...
cross_over = CrossOverEarlyStopping(patience=20)
...
def train_progressively(model, X_train, y_train, X_val, y_val, w_train, w_val,
    ...
    for stage in range(1, depth + 1):
        ...
        # Setup callbacks
        callbacks = [modify_LR, cross_over]  # Use your existing ReduceLROnPlateau callback
