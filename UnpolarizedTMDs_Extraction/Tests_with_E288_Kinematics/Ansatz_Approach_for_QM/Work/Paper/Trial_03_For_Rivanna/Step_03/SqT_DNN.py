...
selected_CS_models_folder = '../Step_02/Models'
CSV_FOLDER = "csvs"
nna_models = "nna_models"
nnb_models = "nnb_models"
SqT_models = "SqT_models"
plots_folder = "training_plots"  # New folder for plots
os.makedirs(CSV_FOLDER, exist_ok=True)
os.makedirs(nna_models, exist_ok=True)
os.makedirs(nnb_models, exist_ok=True)
os.makedirs(SqT_models, exist_ok=True)
os.makedirs(plots_folder, exist_ok=True)  # Create plots directory
...
selected_CS_model_files = [f for f in os.listdir(selected_CS_models_folder) if f.endswith('.h5')]
selected_CS_models_list = [tf.keras.models.load_model(
    os.path.join(selected_CS_models_folder, f), 
    custom_objects={
...
) for f in selected_CS_model_files]

selected_SqT_models_list = [temp_model.get_layer('SqT') for temp_model in selected_CS_models_list]

print(f"Loaded {len(selected_CS_models_list)} SqT models from '{selected_CS_models_folder}'.")
...
n_val_samples = 200  # Added validation samples
# batch_size = 256
batch_size = 32

# Loop over all loaded models
for idx, SqT_model in enumerate(selected_SqT_models_list):
    print(f"\nProcessing model {idx + 1}/{len(selected_SqT_models_list)}")
    ...
    S_train = generate_training_data(SqT_model, x_a_train, x_b_train, q_T_train)
    S_val = generate_training_data(SqT_model, x_a_val, x_b_val, q_T_val)

    # Redefine NN_a and NN_b for each model
    NN_a = make_dnn("NN_a")
    NN_b = make_dnn("NN_b")
    ...
    # Redefine train_step for new NN_a and NN_b in this loop
    @tf.function
    ...
    def evaluate(xa, xb, qt, true_vals):
        inputs = tf.stack([qt, xa, xb], axis=1)
        preds = createModel_S_pred()(inputs, training=False)
        loss = tf.reduce_mean((preds - tf.reshape(true_vals, (-1, 1))) ** 2)
        return loss.numpy()
    ...
    plt.title(f'Model {idx} Training and Validation Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss (MSE)')
    plt.legend()
    plt.grid(True)
    ...
    # Save the plot
    plot_path = os.path.join(plots_folder, f"loss_plot_model_{idx}.png")
    ...
    loss_df.to_csv(os.path.join(plots_folder, f"loss_data_model_{idx}.csv"), index=False)

    # Save the trained models with index
    NN_a.save(f"{nna_models}/NN_a_model_{idx}.h5")
    NN_b.save(f"{nnb_models}/NN_b_model_{idx}.h5")
    createModel_S_pred().save(f"{SqT_models}/SqT_pred_model_{idx}.h5")
    print(f"Models saved for index {idx}: 'NN_a_model_{idx}', 'NN_b_model_{idx}', 'SqT_pred_model_{idx}'")
    ...
    df.to_csv(f"{CSV_FOLDER}/SqT_predictions_{idx}.csv", index=False)
    ...
    plt.title(f'Model {idx}: True vs Predicted Values')
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(plots_folder, f"prediction_scatter_model_{idx}.png"))
    plt.close()

finishtime = datetime.datetime.now().replace(microsecond=0)
totalduration = finishtime - starttime
print(f"\nAll models processed. Total duration --> {totalduration}")

# Create a summary plot of all models' final training and validation losses
final_train_losses = []
final_val_losses = []
model_indices = []

for idx in range(len(selected_SqT_models_list)):
    try:
        loss_data = pd.read_csv(os.path.join(plots_folder, f"loss_data_model_{idx}.csv"))
        final_train_losses.append(loss_data['train_loss'].iloc[-1])
        final_val_losses.append(loss_data['val_loss'].iloc[-1])
        model_indices.append(idx)
    except:
        print(f"Could not load loss data for model {idx}")
...
if model_indices:
    ...
    plt.bar(x - width/2, final_train_losses, width, label='Final Training Loss')
    plt.bar(x + width/2, final_val_losses, width, label='Final Validation Loss')
    
    plt.xlabel('Model Index')
    plt.ylabel('Final Loss (MSE)')
    plt.title('Comparison of Final Losses Across All Models')
    plt.xticks(x, model_indices)
    plt.legend()
    plt.grid(True, axis='y')
    plt.yscale('log')
    
    plt.tight_layout()
    plt.savefig(os.path.join(plots_folder, "all_models_final_loss_comparison.png"))
    plt.close()
    print("Summary plot of all models created.")
...
# n_samples = 1000
# batch_size = 256
# x_a_train, x_b_train, q_T_train = generate_kinematics(n_samples)
...
#     # Evaluation
#     # x_a_test, x_b_test, q_T_test, S_true_test = generate_training_data(1000)
