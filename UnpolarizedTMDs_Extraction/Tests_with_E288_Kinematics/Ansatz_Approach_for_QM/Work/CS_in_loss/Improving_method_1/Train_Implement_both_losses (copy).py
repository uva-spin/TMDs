...
#     a_loss = tf.reduce_mean(tf.square(a_pred - y_true))
#     return a_loss
...
def replica_model(i):
    ...
    x1_values = np.array(replica_data['x1'])
    x2_values = np.array(replica_data['x2'])
    qT_values = tf.constant(replica_data['qT'].values, dtype=tf.float32)
    QM_values =tf.constant(replica_data['QM'].values, dtype=tf.float32)
    ...
    PDFs_values = np.array(replica_data['PDFs'])
    SqT_values = S(qT_values)
    QM_int_values = compute_QM_integrals(QM_values)
    ...
    def custom_loss_wrapper():
        def loss_fn(y_true, y_pred):
            ...
            a_loss = tf.reduce_mean(tf.square(a_pred - A_replica))
            #b_calc = A_replica /(hc_factor * factor_values * PDFs_values * SqT_values * QM_int_values)
            b_loss = tf.reduce_mean(tf.square(y_pred - y_true))
            return 0.0*a_loss + b_loss
        return loss_fn
