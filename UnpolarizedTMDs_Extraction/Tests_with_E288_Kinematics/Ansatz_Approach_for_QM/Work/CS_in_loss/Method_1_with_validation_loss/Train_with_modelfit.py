...
def replica_model(i):
    ...
    # Compile with the custom loss for training
    dnnB.compile(optimizer=optimizer, loss=train_loss)
    
    # Define separate evaluation metric for validation
    # We'll manually calculate validation loss in a callback
    ...
    history = dnnB.fit(
        ...
        verbose=0,  # We'll use our callback for logging
    ...
    # Get validation losses from callback
    # Combine into history for plotting
    history_dict = history.history
    history_dict['val_loss'] = val_callback.val_losses
    
    # Plot Loss
    plt.figure(figsize=(10, 6))
    plt.plot(history_dict['loss'], label='Training Loss', color='b')
