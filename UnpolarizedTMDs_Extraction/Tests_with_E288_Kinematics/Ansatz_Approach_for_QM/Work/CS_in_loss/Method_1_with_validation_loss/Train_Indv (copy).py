...
def makeData_for_split(df):
    ...
    yerr = np.array(df['A_true_err'])

    for key in data.keys():
        features[key] = np.array(data[key])
        
    return features, np.array(y), np.array(yerr)
...
def replica_model(i):
    ...
    QM_values = np.array(train_X['QM'])
    B_QM = np.array(train_X['B_calc'])
    A_replica = np.array(train_A)
    
    factor_values = np.array(train_X['factor'])
    PDFs_values = np.array(train_X['PDFs'])
    SqT_values = np.array(train_X['SqT'])
    QM_int_values = np.array(train_X['QM_int'])
