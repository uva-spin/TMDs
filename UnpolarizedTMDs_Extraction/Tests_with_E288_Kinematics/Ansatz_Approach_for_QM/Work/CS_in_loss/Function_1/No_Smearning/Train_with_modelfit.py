...
def replica_model(i):
    ...
    def custom_loss(y_true, y_pred):
        # Unpack additional inputs from the input list
        factors = y_pred[:, 1]
        pdfs = y_pred[:, 2]
        sqt = y_pred[:, 3]
        qm_int = y_pred[:, 4]
        ...
        a_pred = y_pred_b * hc_factor * factors * pdfs * sqt * qm_int
        return tf.reduce_mean(tf.square(a_pred - y_true))

    # Prepare inputs with additional information
    train_inputs = [train_QM_values]
    train_extended_info = np.column_stack([
        dnnB(train_QM_values),  # B predictions
        train_factor_values,
        train_PDFs_values,
        train_SqT_values,
        train_QM_int_values
    ...
    val_inputs = [test_QM_values]
    val_extended_info = np.column_stack([
        dnnB(test_QM_values),  # B predictions
        test_factor_values,
        test_PDFs_values,
        test_SqT_values,
        test_QM_int_values
    ...
    history = dnnB.fit(
        x=train_inputs, 
        y=train_A_replica,
        validation_data=(val_inputs, test_A_replica),
        epochs=epochs,
