...
dA_values = tf.constant(data['dA'].values, dtype=tf.float32)
A_true_values = tf.constant(data['A'].values, dtype=tf.float32)
# A_rel_uncrt = dA_values / A_true_values
...
def prep_data_for_plots(df):
    ...
    temp_df['A_rel_uncert'] = abs(A_rel_uncrt)
...
processed_data = prep_data_for_plots(data)

QM_values = np.array(processed_data['QM'].unique())
fDNNQ_values = fDNNQ(QM_values)
...
A_rel_uncert_first = processed_data.groupby('QM')['A_rel_uncert'].first().reindex(QM_values).values
