...
# [Most of the previous imports and function definitions remain the same]
# Only the replica_model function will be significantly modified
...
def replica_model(i):
    ...
    X_train = train_QM_values.reshape(-1, 1)
    X_val = test_QM_values.reshape(-1, 1)
    ...
    # Train the model
    history = dnnB.fit(
        X_train, train_A_replica, 
        validation_data=(X_val, test_A_replica),
        epochs=epochs, 
        batch_size=batch_size,
        callbacks=[loss_history],
        verbose=2  # Set to 1 if you want to see training progress
    )
