...
# Define a custom loss function that matches your training loss
def custom_loss_wrapper():
    def loss_fn(y_true, y_pred):
        # This is a placeholder - the actual computation happens in the training code
        # We just need this to match the function signature for loading the model
        return tf.reduce_mean(tf.square(y_pred - y_true))
    return loss_fn

# Define Custom Loss for model loading
def custom_loss(y_true, y_pred):
    # Simple placeholder version just for loading models
    return tf.reduce_mean(tf.square(y_pred - y_true))

# Load All Trained Models with proper custom loss
model_files = [f for f in os.listdir(models_folder) if f.endswith('.h5')]
...
def compute_A(x1, x2, qT, QM):
    # Get Predictions from All Models
    fDNN_contributions = np.array([model.predict(np.array(QM).reshape(-1, 1), verbose=0).flatten() for model in models_list])
    fDNN_mean = np.mean(fDNN_contributions, axis=0)
    fDNN_std = np.std(fDNN_contributions, axis=0)
    ...
    # This matches your training loss calculation - B prediction * all factors = A prediction
    A_pred = fDNN_mean * factor_temp * PDFs_temp * Sk_temp * hc_factor * QM_integral_temp
    A_std = fDNN_std * factor_temp * PDFs_temp * Sk_temp * hc_factor * QM_integral_temp
...
#         n_rows = len(unique_QM) // 2 + len(unique_QM) % 2
        
#         # Create figure with subplots
#         fig, axes = plt.subplots(nrows=n_rows, ncols=2, figsize=(12, 6 * n_rows))
        
#         # Handle case where there's only one QM value
#         if n_rows == 1 and len(unique_QM) == 1:
#             axes = [axes]
#         elif n_rows == 1:
#             axes = axes.flatten()
#         else:
...
#         # Create plots for each QM value
#         for i, QM_val in enumerate(unique_QM):
...
#                 axes[i].errorbar(
#                     prepared_df['qT'][mask], 
#                     prepared_df['A_true'][mask], 
#                     yerr=prepared_df['A_true_err'][mask], 
#                     fmt='bo', 
#                     label=f'QM = {QM_val:.2f} (True)', 
#                     capsize=3
#                 )
#                 axes[i].errorbar(
#                     prepared_df['qT'][mask], 
#                     prepared_df['A_pred'][mask], 
#                     yerr=prepared_df['A_pred_err'][mask], 
#                     fmt='rx', 
#                     label=f'QM = {QM_val:.2f} (Pred)', 
#                     capsize=3
#                 )
#                 axes[i].set_xlabel(r'$q_T$ (GeV)', fontsize=12)
#                 axes[i].set_ylabel(r'$A(q_T)$ (pb/GeV$^2$)', fontsize=12)
#                 axes[i].legend(loc='best')
#                 axes[i].grid(True)
#                 axes[i].set_title(f'QM = {QM_val:.2f} GeV', fontsize=14)
                
#                 # Optional: Set y-axis to start from 0
#                 ylim = axes[i].get_ylim()
#                 axes[i].set_ylim(bottom=0, top=ylim[1]*1.1)
...
#         # Create single page plots for each QM value (optional but helpful)
#         for QM_val in unique_QM:
...
#                 capsize=3
#             )
#             ax.set_xlabel(r'$q_T$ (GeV)', fontsize=14)
#             ax.set_ylabel(r'$A(q_T)$ (pb/GeV$^2$)', fontsize=14)
#             ax.set_title(f'Cross-section at QM = {QM_val:.2f} GeV', fontsize=16)
...
#         print(f"Plots saved successfully in {results_folder}/Comparison_Subplots.pdf")
...
QM_values_reshaped = QM_values.reshape(-1, 1)  # Properly reshape for model input
dnnQ_contributions = np.array([model.predict(QM_values_reshaped, verbose=0).flatten() for model in models_list])
dnnQ_mean = np.mean(dnnQ_contributions, axis=0)
dnnQ_std = np.std(dnnQ_contributions, axis=0)
...
# # Generate QM vs B(QM) plot with better formatting
# def plot_BQM_comparison():
#     # Generate QM Range for Comparison
#     QM_range = np.linspace(data['QM'].min(), data['QM'].max(), 200).reshape(-1, 1)
#     fDNNQ_values = fDNNQ(QM_range.flatten())
    
#     # Get model predictions
#     dnnQ_contributions = np.array([model.predict(QM_range, verbose=0).flatten() for model in models_list])
#     dnnQ_mean = np.mean(dnnQ_contributions, axis=0)
#     dnnQ_std = np.std(dnnQ_contributions, axis=0)
    
#     # Plot analytical vs model predictions
#     fig, ax = plt.subplots(figsize=(10, 6))
#     ax.plot(QM_range.flatten(), fDNNQ_values, label=r'Analytical $\mathcal{B}(Q_M)$', 
#             linestyle='--', color='blue', linewidth=2)
#     ax.plot(QM_range.flatten(), dnnQ_mean, label='Neural Network Model', 
#             linestyle='-', color='red', linewidth=2)
#     ax.fill_between(QM_range.flatten(), dnnQ_mean - dnnQ_std, dnnQ_mean + dnnQ_std, 
#                     color='red', alpha=0.2, label="Model Uncertainty")
    
#     # Plot individual model predictions for transparency (optional)
#     for i, pred in enumerate(dnnQ_contributions):
#         if i < 10:  # Limit to first 10 models to avoid cluttering
#             ax.plot(QM_range.flatten(), pred, color='gray', alpha=0.3, linewidth=0.5)
    
#     ax.set_xlabel(r'$Q_M$ (GeV)', fontsize=14)
#     ax.set_ylabel(r'$\mathcal{B}(Q_M)$', fontsize=14)
#     ax.set_title(r'Comparison of Analytical and Neural Network $\mathcal{B}(Q_M)$', fontsize=16)
#     ax.legend(fontsize=12)
#     ax.grid(True)
...
#     plt.savefig(f"{results_folder}/QM_comparison_plot.pdf")
#     plt.close(fig)
#     print(f"B(QM) plot saved successfully in {results_folder}/QM_comparison_plot.pdf")
