...
# A_mean, A_std = np.mean(pseudo_df["A"]), np.std(pseudo_df["A"])
...
def compute_A(x1, x2, qT, QM):
    ...
    A_pred = A_pred * factor_temp * PDFs_temp * Sk_temp
    A_std_restored = A_std_restored * factor_temp * PDFs_temp * Sk_temp
...
for idx, (group_name, group_df) in enumerate(groups):
    ...
    #factor = ((4*np.pi*alpha)**2)/(9*QM**3)/(2*np.pi)
...
pseudo_df.to_csv(f"{results_folder}/Results_pseudodataE288.csv", index=False)
