...
df = pd.read_csv('E288_pseudo_data_k_0_2_phi_90.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
...
def Generate_Comparison_Data(df, num_replicas, kv):
    ...
    concat_inputs_xA = np.column_stack((Kvals,QMvals))
    concat_inputs_xB = np.column_stack((pT_k_vals,QMvals))
    ...
    for i in range(num_replicas):
        t = modelsArray[i]
        temp_pred_Su = nnq(t, 'nnu', concat_inputs_xA)
        temp_pred_Subar = nnq(t, 'nnubar', concat_inputs_xB)
        tempfu.append(temp_pred_Su)
        tempfubar.append(temp_pred_Subar)

        temp_pred_Su_rev = nnq(t, 'nnu', concat_inputs_xB)
        temp_pred_Subar_rev = nnq(t, 'nnubar', concat_inputs_xA)
    ...
    pred_fbxAk_mean = fubar_xA*(pred_fbxAk_mean.flatten())
