...
# Load the input data from A_for_E288kinematics.csv
data = pd.read_csv("A_for_E288kinematics.csv")
...
def custom_loss(A_true, x1_values, x2_values, pT_values, QM_values, dnn_S1, dnn_S2):
    k = tf.linspace(kmin, kmax, kbins)  # Discretized k values
    phi = tf.linspace(0.0, 2 * np.pi, phibins)  # Discretized phi values
    loss = tf.constant(0.0, dtype=tf.float32)

    for i, (x1, x2, pT, QM) in enumerate(zip(x1_values, x2_values, pT_values, QM_values)):
        ...
        for j, k_val in enumerate(k):
            ...
            # Compute sqrt(pT^2 + k^2 - 2*pT*k*cos(phi))
            cos_phi = tf.cos(phi_tf)
            sqrt_term = tf.sqrt(tf.cast(pT**2, tf.float32) + tf.cast(k_val**2, tf.float32) - 
                                2 * tf.cast(pT, tf.float32) * tf.cast(k_val, tf.float32) * cos_phi)
    ...
    QM_zeros = tf.zeros_like(k_values)  # QM is constant for these integrals

    # Calculate integrals
    S1_integrand = dnn_S1(tf.concat([k_values, QM_zeros], axis=1))
    S2_integrand = dnn_S2(tf.concat([k_values, QM_zeros], axis=1))
...
for epoch in range(epochs):
    with tf.GradientTape() as tape:
        loss = custom_loss(A_true, x1_values, x2_values, pT_values, QM_values, dnn_S1, dnn_S2)
