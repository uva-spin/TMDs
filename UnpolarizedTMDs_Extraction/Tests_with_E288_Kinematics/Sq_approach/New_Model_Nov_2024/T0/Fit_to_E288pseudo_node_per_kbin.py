...
df = pd.read_csv('E288_pseudo_data_k_0_2_phi_0.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
...
def create_nn_model(name):
    inp = tf.keras.Input(shape=(kBins))
    initializer = tf.keras.initializers.RandomUniform(minval=-0.1,maxval=0.1,seed=42)
    x = tf.keras.layers.Dense(kBins, activation='relu6', kernel_initializer = initializer, kernel_regularizer=tf.keras.regularizers.L1(L1_reg))(inp)
    ...
    nnout = tf.keras.layers.Dense(1, activation='relu6', kernel_initializer = initializer, kernel_regularizer=tf.keras.regularizers.L1(L1_reg))(x)
...
def createModel_DY():
    ...
    return tf.keras.Model(inputs=[x1, x2, qT, qM,fuxA,fubarxA,fuxB,fubarxB], outputs=[tmd_product_sum])
...
#    history = model.fit([trainKin['x1'],trainKin['x2'],trainKin['pT'],trainKin['QM'],trainKin['fu_xA'],trainKin['fubar_xA'],trainKin['fu_xB'],trainKin['fubar_xB']], [trainA, trainfq, trainfqbar, trainfq_rev, trainfqbar_rev],  validation_data=([testKin['x1'],testKin['x2'],testKin['pT'],testKin['QM'],testKin['fu_xA'],testKin['fubar_xA'],testKin['fu_xB'],testKin['fubar_xB']], [testA, testfq, testfqbar, testfq_rev, testfqbar_rev]), epochs=EPOCHS, batch_size=BATCH, verbose=2)
   ...
   # Creating arrays of ones with the length of the training and testing data
    ...
    # ones_train = 1*np.ones_like(trainKin['x1'])  # Array of ones with the length of training data
    # ones_test = 1*np.ones_like(testKin['x1'])    # Array of ones with the length of testing data
    ...
    history = model.fit(
        [trainKin['x1'], trainKin['x2'], trainKin['pT'], trainKin['QM'], trainKin['fu_xA'], trainKin['fubar_xA'], trainKin['fu_xB'], trainKin['fubar_xB']],
        [trainA], validation_data=( [testKin['x1'], testKin['x2'], testKin['pT'], testKin['QM'], testKin['fu_xA'], testKin['fubar_xA'], testKin['fu_xB'], testKin['fubar_xB']],
