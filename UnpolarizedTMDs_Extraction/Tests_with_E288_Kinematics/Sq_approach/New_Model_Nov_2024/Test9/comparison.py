...
QM_test = np.full_like(k_test, 10)  # Example QM value

# Get predictions from the models (DNN takes only `k` as input)
S1_pred = dnn_S1.predict(k_test).flatten() * np.log(100 / QM_test.flatten())
S2_pred = dnn_S2.predict(k_test).flatten() * np.log(100 / QM_test.flatten())
