...
# Define the DNN model with only k as input
def build_dnn():
    model = models.Sequential([
        layers.Input(shape=(1,)),  # Only one input: k
        ...
        layers.Dense(1, activation='exponential')  # Output S(k)
    ])
    return model

# Instantiate DNNs for S1(k) and S2(k)
dnn_S1 = build_dnn()
dnn_S2 = build_dnn()
...
def custom_loss(A_true, qT_values, QM_values, dnn_S1, dnn_S2):
    ...
    for i, (qT, QM) in enumerate(zip(qT_values, QM_values)):
        ...
        for j, k_val in enumerate(k):
            ...
            # Compute outputs of DNNs
            S1_output = dnn_S1(k_tf) * tf.cast(tf.math.log(100.0 / QM), tf.float32)
            S2_output = dnn_S2(sqrt_term) * tf.cast(tf.math.log(100.0 / QM), tf.float32)

            # Compute integrand
            term1 = S1_output * S2_output
            term2 = S1_output * S2_output
