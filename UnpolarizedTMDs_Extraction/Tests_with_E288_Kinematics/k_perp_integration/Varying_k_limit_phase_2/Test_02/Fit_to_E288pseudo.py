...
models_path = '/scratch/cee9hc/Unpolarized_TMD/E288/flavor_1/Phase_2/Test_02'
...
def createModel_DY():
    ...
    for k_val in k_values:
        ...
        xA_k_input = tf.keras.layers.Concatenate()([x1, qT * 0 + k_val, qM])
        xB_k_input = tf.keras.layers.Concatenate()([x2, qT * 0 + k_val, qM])
        f_xA.append(modnnu(xA_k_input))
        fbar_xB.append(modnnubar(xB_k_input))
    ...
    return tf.keras.Model(inputs=[x1, x2, qT, qM], outputs=[tmd_product_sum, f_xA_sum, fbar_xB_sum, f_xB_sum, fbar_xA_sum])
