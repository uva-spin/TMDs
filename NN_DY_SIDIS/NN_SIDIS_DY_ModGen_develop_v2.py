...
### In the custom fit function, we consider model1 as SIDIS and model2 as DY ###

def customfit(model1, model2, epochs, X1, y1, sample_weights1, X1v, y1v, err1v, X2, y2, sample_weights2, X2v, y2v, err2v, loss_fn, optimizer, batch_size=4):
    #epochs = 2
    for epoch in range(epochs):
        ...
        for step in range((len(y1)+len(y2))//batch_size):
            ...
            with tf.GradientTape() as tape:
                ...
                y_hat_val= model(x_val, training = False)

                # Compute the loss value for this minibatch.
                loss_value = loss_fn(y_batch, y_hat, sample_weight=sample_weights_batch)

                loss_validation = loss_fn(y_val, y_hat_val, sample_weight=err_val) 
            ...
            #print(" Training loss is  %.4f" % loss_value)
            #print(" Validation loss is  %.4f" % loss_validation)
...
def trainReplicas_new(epochs, X1, y1, err1, X1v, y1v, err1v, X2, y2, err2, X2v, y2v, err2v, loss_fn, optimizer, numReplicas, batch_size=4):
    for i in range(numReplicas):
        ...
        #customfit(model1, model2, epochs, X1, y1, sample_weights1, X1v, y1v, err1v, X2, y2, sample_weights2, X2v, y2v, err2v, loss_fn, optimizer, batch_size=4)
        customfit(sivModel_SIDIS, sivModel_DY, epochs, X1, yrep1, np.square(1/err1), X1v, y1v, np.square(1/err1v), X2, yrep2, np.square(1/err2), X2v, y2v, np.square(1/err2v), loss_fn, optimizer, batch_size)
...
if __name__=='__main__': ## double underscore means dundered 
    ...
    #trainReplicas_new(epochs, X1, y1, err1, X1v, y1v, err1v, X2, y2, err2, X2v, y2v, err2v, loss_fn, optimizer, numReplicas, batch_size=4)
    trainReplicas_new(10, X_SIDIS, y_SIDIS, err_SIDIS, tst_X_SIDIS, tst_y_SIDIS, tst_err_SIDIS, X_DY, y_DY, err_DY, tst_X_DY, tst_y_DY, tst_err_DY, loss_fn, optimizer, 2, batch_size=15)
