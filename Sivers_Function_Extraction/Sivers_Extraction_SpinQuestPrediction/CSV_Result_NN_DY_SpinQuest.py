...
Models_folder = '/scratch/cee9hc/Sivers_NN_Models/Train_Real/Fit3/Models_DY_wRD'
folders_array=os.listdir(Models_folder)
#folders_array=natsort.natsorted(folders_array)
OutputFolder='NN_DY_wRD_Fit_Results'
...
DY_SPINQUEST_DATA = pd.read_csv('./Data/SpinQuest_Kinematics_xF_dependence.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
DY_SPINQUEST_DATA_df = pd.concat([DY_SPINQUEST_DATA])



df = pd.concat([DY_SPINQUEST_DATA])
...
NN_Pseudo_DY_from_SIDIS_minus=Project_DY_from_Models(df, numreplicas_DY, 1)

#NN_Pseudo_DY_from_SIDIS_plus.to_csv(str(OutputFolder)+'/Result_DY_from_SIDIS_plus.csv')
NN_Pseudo_DY_from_SIDIS_minus.to_csv(str(OutputFolder)+'/Result_DY_from_SIDIS_minus_for_e1039.csv')
