...
PseudoData_Set = "../../Pseudo_Data/Set_1"

## DY ###
DY_Dat1=str(PseudoData_Set)+'/Pseudo_DY_COMPASS2017.csv'

## SIDIS ###
SIDIS_Dat1=str(PseudoData_Set)+'/Pseudo_SIDIS_HERMES2009.csv'
SIDIS_Dat2=str(PseudoData_Set)+'/Pseudo_SIDIS_HERMES2020.csv'
SIDIS_Dat3=str(PseudoData_Set)+'/Pseudo_SIDIS_COMPASS2009.csv'
SIDIS_Dat4=str(PseudoData_Set)+'/Pseudo_SIDIS_COMPASS2015.csv'
    ...
    for ep in range(epchs):
        ...
        for step in range((len(y1)+len(y2))//batch_size):
            ...
            with tf.GradientTape() as tape:
                ...
                #print(X1Val)
                Train_loss_step_array.append(loss_value)
        ...
        mean_train_loss.update_state(Train_loss_step_array)
        mean_train_loss.result().numpy
...
if __name__=='__main__': ## double underscore means dundered 
    ...
    herm9_SIDIS = pd.read_csv(str(SIDIS_Dat1)).dropna(axis=0, how='all').dropna(axis=1, how='all')
    herm20_SIDIS = pd.read_csv(str(SIDIS_Dat2)).dropna(axis=0, how='all').dropna(axis=1, how='all')
    comp9_SIDIS = pd.read_csv(str(SIDIS_Dat3)).dropna(axis=0, how='all').dropna(axis=1, how='all')
    comp15_SIDIS = pd.read_csv(str(SIDIS_Dat4)).dropna(axis=0, how='all').dropna(axis=1, how='all')

    ### DY ###

    comp15_DY = pd.read_csv(str(DY_Dat1)).dropna(axis=0, how='all').dropna(axis=1, how='all')
