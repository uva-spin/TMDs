...
class Sivers_Hadron(Hadron):
    ...
    def sivers(self, had, kins, m1, Nu, au, bu, Nub, aub, bub, Nd, ad, bd, Ndb, adb, bdb, NS, aS, bS, NSb, aSb, bSb):
        if had == 'pi+':
            ii = 0
        elif had == 'pi-':
            ii = 1
        ...
        QQ = kins[:, 3]
        a0 = self.A0(z, pht, m1)
        temp_top = NN(x, Nu, au, bu) * self.eu**2 * self.pdf(2, x, QQ) * self.ff(self.ffDict[ii],2, z, QQ)
        + NNanti(x, Nub, aub, bub) * self.eubar**2 * self.pdf(-2, x, QQ) * self.ff(self.ffDict[ii],-2, z, QQ)
        + NN(x, Nd, ad, bd) * self.ed**2 * self.pdf(1, x, QQ) * self.ff(self.ffDict[ii],1, z, QQ)
        + NNanti(x, Ndb, adb, bdb) * self.edbar**2 * self.pdf(-1, x, QQ) * self.ff(self.ffDict[ii],-1, z, QQ) 
        + NN(x, NS, aS, bS) * self.es**2 * self.pdf(3, x, QQ) * self.ff(self.ffDict[ii],3, z, QQ)
        + NNanti(x, NSb, aSb, bSb) * self.esbar**2 * self.pdf(-3, x, QQ) * self.ff(self.ffDict[ii],-3, z, QQ)
...
class Sivers_DY(DY_Hadron):
    ...
    def sivers(self, kins, m1, Nu, au, bu, Nub, aub, bub, Nd, ad, bd, Ndb, adb, bdb, NS, aS, bS, NSb, aSb, bSb):
        x1 = kins[:, 0]
        x2 = kins[:, 1]
        qT = kins[:, 2]
        QQ = kins[:, 3] 
        b0 = self.B0(qT, m1)
        temp_top = NN(x1, Nu, au, bu) * self.eu**2 * self.pdf_DY(2, x1, QQ) * self.pdf_DY(-2, x2, QQ)
        + NN(x2, Nu, au, bu) * self.eu**2 * self.pdf_DY(2, x2, QQ) * self.pdf_DY(-2, x1, QQ)
        + NNanti(x1, Nub, aub, bub) * self.eubar**2 * self.pdf_DY(-2, x1, QQ) * self.pdf_DY(2, x2, QQ)
        + NNanti(x2, Nub, aub, bub) * self.eubar**2 * self.pdf_DY(-2, x2, QQ) * self.pdf_DY(2, x1, QQ)
        + NN(x1, Nd, ad, bd) * self.ed**2 * self.pdf_DY(1, x1, QQ) * self.pdf_DY(-1, x2, QQ)
        + NN(x2, Nd, ad, bd) * self.ed**2 * self.pdf_DY(1, x2, QQ) * self.pdf_DY(-1, x1, QQ)
        + NNanti(x1, Ndb, adb, bdb) * self.edbar**2 * self.pdf_DY(-1, x1, QQ) * self.pdf_DY(1, x2, QQ)
        + NNanti(x2, Ndb, adb, bdb) * self.edbar**2 * self.pdf_DY(-1, x2, QQ) * self.pdf_DY(1, x1, QQ)
        + NN(x1, NS, aS, bS) * self.es**2 * self.pdf_DY(3, x1, QQ) * self.pdf_DY(-3, x2, QQ)
        + NN(x2, NS, aS, bS) * self.es**2 * self.pdf_DY(3, x2, QQ) * self.pdf_DY(-3, x1, QQ)
        + NNanti(x1, NSb, aSb, bSb) * self.esbar**2 * self.pdf_DY(-3, x1, QQ) * self.pdf_DY(3, x2, QQ)
        + NNanti(x2, NSb, aSb, bSb ) * self.esbar**2 * self.pdf_DY(-3, x2, QQ) * self.pdf_DY(3, x1, QQ)
