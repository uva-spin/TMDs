...
def plotSivAsymm(datasetdf,PSdatasetdf,hadron,dependence, numSigma=2):
    D_Xplt, D_DEP, D_yplt, D_errplt = datann.makeData(datasetdf, [hadron], [dependence])
    PS_Xplt, PS_DEP, PS_yplt, PS_errplt = datann.makeData(PSdatasetdf, [hadron], [dependence])
    plt.errorbar(D_DEP, D_yplt,yerr=D_errplt, fmt='bo',label='Data')
    plt.errorbar(PS_DEP, PS_yplt,yerr=PS_errplt, fmt='ro',label='Pseudo_Data')
    # #### Here the user needs to define the plot title based on the data set ####
    plt.title('Data vs PseudoData'+str(hadron),fontsize=15)
    plt.xlabel(str(dependence),fontsize=15)
    plt.legend(loc=2,fontsize=15,handlelength=3)
    miny=np.min(D_yplt)
    maxy=np.max(D_yplt)
    delta=maxy-miny
    plt.ylim([miny-2*delta,maxy+2*delta])
    #return PS_Xplt, PS_DEP, PS_yplt, PS_errplt
...
#### Here the user needs to define the plot title based on the data set ####
plt.savefig('HERMES20_comparison.pdf', format='pdf', bbox_inches='tight')
...
#### Here the user needs to define the plot title based on the data set ####
plt.savefig('COMPASS09_comparison.pdf', format='pdf', bbox_inches='tight')
...
#### Here the user needs to define the plot title based on the data set ####
plt.savefig('COMPASS15_comparison.pdf', format='pdf', bbox_inches='tight')
