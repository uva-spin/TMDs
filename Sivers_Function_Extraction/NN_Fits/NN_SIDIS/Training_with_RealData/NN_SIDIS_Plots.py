...
def xsivdistFromReplicas(numReplicas, x, QQ, kperp2avg, flavor, kperp):
    ...
    for i in range(numReplicas):
        t = tf.keras.models.load_model('Models_SIDIS_100/rep' + str(i) + '.h5', 
...
def plotSivDistBands(numReplicas, x, QQ, kperp2avg, flavor, kperp, numSigma=2):
    ...
    plt.savefig('SiversQ_SIDIS_NN_50rep.pdf', format='pdf', bbox_inches='tight')
...
#     plt.savefig('2009HermesUDSsea.pdf', format='pdf', bbox_inches='tight')
    ...
    plt.savefig('SiversAntiQ_SIDIS_NN_50rep.pdf', format='pdf', bbox_inches='tight')    
    
f1 = plt.figure(1)
plotSivDistBands(50, 0.1, 2.4, .57, -2, np.array(list(range(150)))/100)
f2 = plt.figure(2)
plotSivDistBandsSea(50, 0.1, 2.4, .57, -2, np.array(list(range(150)))/100)