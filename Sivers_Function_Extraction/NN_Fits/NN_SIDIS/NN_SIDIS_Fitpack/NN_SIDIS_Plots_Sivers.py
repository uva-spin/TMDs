...
Models_folder = 'Models_SIDIS_central'
...
def xsivdistFromReplicas(numReplicas, x, QQ, kperp2avg, flavor, kperp):
    ...
    for i in range(numReplicas):
        t = tf.keras.models.load_model(str(Models_folder)+'/rep' + str(i) + '.h5', 
                                         custom_objects={'A0': A0, 'Quotient': Quotient})
        results.append(list(xsivdist(t, x, QQ, kperp2avg, flavor, kperp)))
    return np.array(results)


def plotSivDistBands(numReplicas, x, QQ, kperp2avg, flavor, kperp, numSigma=1):
    #datann = DataANN()
    ...
    plt.savefig('SiversQ_SIDIS_NN_c.pdf', format='pdf', bbox_inches='tight')
    
    
def plotSivDistBandsSea(numReplicas, x, QQ, kperp2avg, flavor, kperp, numSigma=1):
    #datann = DataANN()
    #X, y, err = datann.makeData(df, [hadron], [dependence])
...
#     plt.savefig('2009HermesUDSsea.pdf', format='pdf', bbox_inches='tight')
    ...
    plt.savefig('SiversAntiQ_SIDIS_NN_c.pdf', format='pdf', bbox_inches='tight')    
    
f1 = plt.figure(1)
plotSivDistBands(numreplicas_SIDIS, 0.1, 2.4, .57, -2, np.array(list(range(150)))/100)
f2 = plt.figure(2)
plotSivDistBandsSea(numreplicas_SIDIS, 0.1, 2.4, .57, -2, np.array(list(range(150)))/100)