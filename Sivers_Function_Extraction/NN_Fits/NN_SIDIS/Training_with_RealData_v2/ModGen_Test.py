...
save_path = '/scratch/cee9hc/Sivers_NN_Models/Train_Real/Models_SIDIS_v5/rep'
Org_Data_path = 'Data/'
...
EarlyStop = tf.keras.callbacks.EarlyStopping(monitor='loss',min_delta = 0.001, patience=1000)
...
def run_replica():
    ...
    #history = sivModel.fit(trn_X, trn_y, sample_weight=(1/trn_err**2), validation_data=(tst_X, tst_y), epochs=EPOCHS, callbacks=[modify_LR,EarlyStop], batch_size=300, verbose=2)
    history = sivModel.fit(trn_X, trn_y, sample_weight=(1/trn_err**2), validation_data=(tst_X, tst_y), epochs=EPOCHS, verbose=2)
