...
RepFolder = Replica_Output_Folder
RepFilesArray = os.listdir(RepFolder)
#RepFilesArray = natsort.natsorted(RepFilesArray )
...
for i in range(NNrep):
    ...
    sivModel.compile(
        optimizer = tf.keras.optimizers.Adam(Learning_Rate),
        #optimizer = tf.keras.optimizers.SGD(lr=Learning_Rate, momentum=0.9, decay=0.01),
        loss = tf.keras.losses.MeanSquaredError()
        )
