...
def SiversAsym(df,numReplicas,hadron,dependence):
    T_Kins, T_Xplt, T_DEP, T_yplt, T_errplt = datann.makeData(df, [hadron], [dependence])
    results = []
...
def GenPseudo(datasetdf, numReplicas, numSigma=1):
    ...
    for i in temp_hads:
        for j in temp_deps:
            T_Kins, T_Xplt, T_DEP, T_yplt, T_errplt = datann.makeData(datasetdf, [str(i)], [str(j)])
...
GenPseudo(COMPASS2009, numreplicas_SIDIS, numSigma=1)



###########################################################
data_path = 'Models_SIDIS'
