...
def SiversAsym(df,numReplicas,hadron,dependence):
    ...
    for i in range(numReplicas):
        ...
        testmodel = tf.keras.models.load_model(str(Models_folder) + '/' + folders_array[i],custom_objects={'A0': A0, 'Quotient': Quotient})
        results.append(list(calc_yhat(testmodel, T_Xplt)))
    return np.array(results)


def plotSivAsymmBands(datasetdf, numReplicas,hadron,dependence, numSigma=1):
