...
#################################################################
save_path = 'Models_SIDIS_central/rep'
...
# def create_nn(name, hidden_layers=Hidden_Layers, width=Nodes_per_HL, activation='tanh'):
...
def create_nn(name, hidden_layers=Hidden_Layers, width=Nodes_per_HL, activation='tanh'):
    ...
    x = tf.keras.layers.Dense(width, activation=activation, kernel_initializer = initializer)(inp)
    for i in range(hidden_layers-1):
        x = tf.keras.layers.Dense(width, activation='relu', kernel_initializer = initializer)(x)
        #xdrop = tf.keras.layers.Dropout((0.25))(x)
    nnout = tf.keras.layers.Dense(1, kernel_initializer = initializer)(x)
    mod = tf.keras.Model(inp, nnout, name=name)
    return mod
...
def run_replica(i):
    ...
    #, sample_weight=(1/trn_err**2)
    history = sivModel.fit(trn_X, trn_y, validation_data=(tst_X, tst_y), epochs=EPOCHS, verbose=2)
