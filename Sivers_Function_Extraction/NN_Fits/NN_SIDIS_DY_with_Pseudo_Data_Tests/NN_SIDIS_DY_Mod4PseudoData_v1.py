#######################################################################
############ Generating Replicas for NN model : SIDIS & DY ############
############ Written by Ishara Fernando & Nick Newton #################
############   Last upgrade: January-2022   ###########################
#######################################################################
...
#     nnqval = nnq(model, np.array([x]), flavor)
#     return ((nnqval)[0, :])
...
def customfit(model1, model2, epochs, X1, y1, sample_weights1, X2, y2, sample_weights2, loss_fn, optimizer, batch_size=4):
    #epochs = 2
    for epoch in range(epochs):
        ...
        for step in range((len(y1)+len(y2))//batch_size):
            ...
            with tf.GradientTape() as tape:
                ...
                model.fit(x_batch, y_batch, sample_weight=sample_weights_batch, epochs=EPOCHS, verbose=2)
...
def trainReplicas_new(epochs, X1, y1, err1, X2, y2, err2, loss_fn, optimizer, numReplicas, batch_size=4):
    for i in range(numReplicas):
        ...
        customfit(sivModel_SIDIS, sivModel_DY, epochs, X1, yrep1, np.square(1/err1), X2, yrep2, np.square(1/err2), loss_fn, optimizer, batch_size)
...
if __name__=='__main__': ## double underscore means dundered 
    ...
    #herm9_SIDIS = pd.read_csv('./Pseudo_Data/Pseudo_SIDIS_HERMES2009.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
    herm20_SIDIS = pd.read_csv('./Pseudo_Data/Pseudo_SIDIS_HERMES2020.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
    comp9_SIDIS = pd.read_csv('./Pseudo_Data/Pseudo_SIDIS_COMPASS2009.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
    comp15_SIDIS = pd.read_csv('./Pseudo_Data/Pseudo_SIDIS_COMPASS2015.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')

    ### DY ###

    comp15_DY = pd.read_csv('./Pseudo_Data/Pseudo_DY_COMPASS2017.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
    ...
    trainReplicas_new(10, X_SIDIS, y_SIDIS, err_SIDIS, X_DY, y_DY, err_DY, loss_fn, optimizer, 2, batch_size=4)
