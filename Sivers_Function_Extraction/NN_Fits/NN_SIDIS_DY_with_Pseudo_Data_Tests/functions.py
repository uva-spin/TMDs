#######################################################################
############ Definitions for NN model : SIDIS & DY    #################
...
class Hadron(object):
    ...
    def ffPIp(self, flavor, z, QQ):
        return np.array([self.ffDataPIp.xfxQ2(flavor, az, qq) for az, qq in zip(z, QQ)])

    def ffPIm(self, flavor, z, QQ):
        return np.array([self.ffDataPIm.xfxQ2(flavor, az, qq) for az, qq in zip(z, QQ)])

    def ffPIsum(self, flavor, z, QQ):
        return np.array([self.ffDataPIsum.xfxQ2(flavor, az, qq) for az, qq in zip(z, QQ)])
        
    def ffKAp(self, flavor, z, QQ):
        return np.array([self.ffDataKAp.xfxQ2(flavor, az, qq) for az, qq in zip(z, QQ)])
    
    def ffKAm(self, flavor, z, QQ):
        return np.array([self.ffDataKAm.xfxQ2(flavor, az, qq) for az, qq in zip(z, QQ)])
...
class PiPlus(Hadron):
    ...
    def sivers(self, kins, Nu, au, bu, Ndbar, m1):  # adbar, bdbar): are not necessary
        ...
        QQ = kins[:, 3]
        a0 = self.A0(z, pht, m1)
        topleft = self.NN(x, Nu, au, bu) * self.eu**2 * self.pdf(2, x, QQ) * self.ff_PIp(2, z, QQ)
        topright = self.NNanti(Ndbar) * self.edbar**2 * self.pdf(-1, x, QQ) * self.ff_PIp(-1, z, QQ)
        bottomleft = self.eu**2 * self.pdf(2, x, QQ) * self.ff_PIp(2, z, QQ)
        bottomright = self.edbar**2 * self.pdf(-1, x, QQ) * self.ff_PIp(-1, z, QQ)
        return a0*((topleft + topright)/(bottomleft + bottomright))
    
    
class PiMinus(Hadron):
    def __init__(self, kperp2avg=.57, pperp2avg=.12, pdfset='cteq61',
                 ff_PIm='NNFF10_PIm_nlo'):
        '''
    ...
    def sivers(self, kins, Nd, ad, bd, Nubar, m1): # aubar, bubar
        ...
        QQ = kins[:, 3]
        a0 = self.A0(z, pht, m1)
        topleft = self.NN(x, Nd, ad, bd) * self.ed**2 * self.pdf(1, x, QQ) * self.ff_PIm(1, z, QQ)
        topright = self.NNanti(Nubar) * self.eubar**2 * self.pdf(-2, x, QQ) * self.ff_PIm(-2, z, QQ)
        bottomleft = self.ed**2 * self.pdf(1, x, QQ) * self.ff_PIm(1, z, QQ)
        bottomright = self.eubar**2 * self.pdf(-2, x, QQ) * self.ff_PIm(-2, z, QQ)
        return a0*((topleft + topright)/(bottomleft + bottomright))
    
    
class PiZero(Hadron):
    def __init__(self, kperp2avg=.57, pperp2avg=.12, pdfset='cteq61',
                 ff_PIsum='NNFF10_PIsum_nlo'):
        '''
    ...
    def sivers(self, kins, Nu, au, bu, Nubar, m1): # aubar, bubar
        ...
        topleft = self.NN(x, Nu, au, bu) * self.eu**2 * self.pdf(2, x, QQ) * (self.ff_PIsum(2, z, QQ))
        # N_ubar * e_ubar^2 * f_ubar(x) * (D_pi+/ubar(z) + D_pi-/ubar(z))
        topright = self.NNanti(Nubar) * self.eubar**2 * self.pdf(-2, x, QQ) * (self.ff_PIsum(-2, z, QQ))
        # e_u^2 * f_u(x) * (D_pi+/u(z) + D_pi-/u(z))
        bottomleft = self.eu**2 * self.pdf(2, x, QQ) * (self.ffPion(2, z, QQ) + self.ff_PIsum(2, z, QQ))
        # e_ubar^2 * f_ubar(x) * (D_pi+/ubar(z) + D_pi-/ubar(z))
        bottomright = self.eubar**2 * self.pdf(-2, x, QQ) * (self.ffPion(-2, z, QQ) + self.ff_PIsum(-2, z, QQ))
        return a0*((topleft + topright)/(bottomleft + bottomright))
    
    
class KPlus(Hadron):
    def __init__(self, kperp2avg=.57, pperp2avg=.12, pdfset='cteq61',
                 ff_KAp='NNFF10_KAp_nlo'):
        '''
    ...
    def sivers(self, kins, Nu, au, bu, Nsbar, m1): # asbar, bsbar
        ...
        topleft = self.NN(x, Nu, au, bu) * self.eu**2 * self.pdf(2, x, QQ) * self.ff_KAp(2, z, QQ)
        # N_sbar * e_sbar^2 * f_sbar(x) * (D_k+/sbar(z) + D_k-/sbar(z))
        topright = self.NNanti(Nsbar) * self.esbar**2 * self.pdf(-3, x, QQ) * self.ff_KAp(-3, z, QQ)
        # e_u^2 * f_u(x) * (D_pi+/u(z) + D_pi-/u(z))
        bottomleft = self.eu**2 * self.pdf(2, x, QQ) * self.ff_KAp(2, z, QQ)
        # e_sbar^2 * f_sbar(x) * (D_pi+/sbar(z) + D_pi-/sbar(z))
        bottomright = self.esbar**2 * self.pdf(-3, x, QQ) * self.ff_KAp(-3, z, QQ)
        return a0*((topleft + topright)/(bottomleft + bottomright))
    
    
class KMinus(Hadron):
    def __init__(self, kperp2avg=.57, pperp2avg=.12, pdfset='cteq61',
                 ff_KAm='NNFF10_KAm_nlo'):
        '''
    ...
    def sivers(self, kins, Ns, as0, bs, Nubar, m1): # asbar, bsbar
        ...
        topleft = self.NN(x, Ns, as0, bs) * self.es**2 * self.pdf(3, x, QQ) * self.ff_KAm(3, z, QQ)
        # N_ubar * e_ubar^2 * f_ubar(x) * (D_k+/ubar(z) + D_k-/ubar(z))
        topright = self.NNanti(Nubar) * self.eubar**2 * self.pdf(-2, x, QQ) * self.ff_KAm(-2, z, QQ)
        # e_s^2 * f_s(x) * (D_pi+/s(z) + D_pi-/s(z))
        bottomleft = self.es**2 * self.pdf(3, x, QQ) * self.ff_KAm(3, z, QQ)
        # e_ubar^2 * f_ubar(x) * (D_pi+/ubar(z) + D_pi-/ubar(z))
        bottomright = self.eubar**2 * self.pdf(-2, x, QQ) * self.ff_KAm(-2, z, QQ)
        return a0*((topleft + topright)/(bottomleft + bottomright))
