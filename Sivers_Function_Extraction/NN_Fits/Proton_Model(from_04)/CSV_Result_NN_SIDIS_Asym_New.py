...
#Org_Data_path = '../PseudoData_from_NN_01/'
...
herm20_3D_DATA = pd.read_csv('../Data/HERMES_2020_3D.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
...
HERMES2020_3D = pd.concat([herm20_3D_DATA])
...
class DataSIDIS_3D(object):
    ...
    def makeData(self, df, hadrons):
        ...
        return kins, data, kins, np.array(y), np.array(err)
        
        
    
SIDISdatann = SIDISDataANN()
SIDISdatann3D = DataSIDIS_3D()
...
def SiversAsym(df,numReplicas,hadron,dependence):
    ...
    for i in range(numReplicas):
        ...
        #print(calc_yhat(testmodel, T_Xplt))
        results.append(list(calc_yhat(testmodel, T_Xplt)))
    return np.array(results)


def SiversAsym3D(df,numReplicas,hadron):
    T_Kins, T_Xplt, T_DEP, T_yplt, T_errplt = SIDISdatann3D.makeData(df, [hadron])
    results = []
    for i in range(numReplicas):
        #folders_array
        #testmodel = tf.keras.models.load_model(str(Models_folder)+'/' + str(folders_array[i]),custom_objects={'A0': A0, 'Quotient': Quotient})
...
# testval= calc_yhat(testmodel, T_Xplt))
#         results.append(list(calc_yhat(testmodel, T_Xplt)))
...
def GenPseudo3D(datasetdf, numReplicas, numSigma=1):
    data_dictionary = {'hadron':[],
                      'Q2': [],
                      'x': [],
                      'z': [],
    ...
    SiversA_err = []
    for i in temp_hads:
        T_Kins, T_Xplt, T_DEP, T_yplt, T_errplt = SIDISdatann3D.makeData(datasetdf, [str(i)])
        results = SiversAsym3D(datasetdf, numReplicas,i)
        Yhat = np.array(results.mean(axis=0))
        Yhat = Yhat.flatten()
        Yerr = np.array(results.std(axis=0))
        Yerr = Yerr.flatten()
...
NN_Pseudo_SIDIS_JLAB2011=GenPseudo(JLAB2011, numreplicas_SIDIS, numSigma=1)
NN_Pseudo_SIDIS_JLAB2011.to_csv(str(OutputFolder)+'/Result_SIDIS_JLAB2011.csv')
...
# NN_Pseudo_SIDIS_HERMES2020_3D=GenPseudo3D(HERMES2020_3D, numreplicas_SIDIS, numSigma=1)
# NN_Pseudo_SIDIS_HERMES2020_3D.to_csv(str(OutputFolder)+'/Result_SIDIS_HERMES2020_3D.csv')




#NN_COMPASS_DY_from_SIDIS_plus=Project_COMPASS_DY(COMPASS_DY2017, numreplicas_SIDIS, 1)
#NN_COMPASS_DY_from_SIDIS_plus.to_csv(str(OutputFolder)+'/Result_COMPASS_DY_from_SIDIS_plus.csv')

#NN_COMPASS_DY_from_SIDIS_minus=Project_COMPASS_DY(COMPASS_DY2017, numreplicas_SIDIS,-1)
#NN_COMPASS_DY_from_SIDIS_minus.to_csv(str(OutputFolder)+'/Result_COMPASS_DY_from_SIDIS_minus.csv')
