...
##### Here we can define the number of nodes in each layer ###
Nodes_L1 = 10
Nodes_L2 = 20
Nodes_L3 = 10

##### Here we can define the activation in each layer ###
Active_L1 = 'relu'
Active_L2 = 'tanh'
Active_L3 = 'relu'
Active_L4 = 'linear'  # Output layer #
...
def create_nn_model(name):
    ...
    L1 = tf.keras.layers.Dense(Nodes_L1, activation=Active_L1)(inp)
    L2 = tf.keras.layers.Dense(Nodes_L2, activation=Active_L2)(L1)
    L3 = tf.keras.layers.Dense(Nodes_L3, activation=Active_L3)(L2)
    nnout = tf.keras.layers.Dense(1, activation=Active_L4)(L3)
...
class SIDIS_DataANN(object):
    ...
    #SIDIS_xFxQ2(datafile,flavor,hadron,dep)
    ...
    for epoch in range(epchs):
        ...
        for step in range((len(y1)+len(y2))//batch_size):
            ...
            if loss_value < Batch_Size:
                print("Training loss saved at step %d: %.4f" % (step, float(loss_value)))
                if ismodel1:
                    model.save('SIDISmodels/rep' + str(step) + '.h5', save_format='h5')
                else:
                    model.save('DYmodels/rep' + str(step) + '.h5', save_format='h5')
