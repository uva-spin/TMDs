...
def Pseudo_Sivers_AntiQ_Plots(x, n, m1, QQ, kperp2avg, kperp):
    kp = kperp
    SivU=Pseudo_SiversAntiQ(x, n, m1, QQ, kperp2avg, kperp, 2)
    plt.plot(kperp, SivU,'--', color='lightblue', label='$\\bar{u}_{true}$')
    SivD=Pseudo_SiversAntiQ(x, n, m1, QQ, kperp2avg, kperp, 1)
    plt.plot(kperp, SivD,'--', color='tomato', label='$\\bar{d}_{true}$')
    SivS=Pseudo_SiversAntiQ(x, n, m1, QQ, kperp2avg, kperp, 3)
...
plt.figure(1)    
plotSivDistBands(1, 0.1, 2.4, .57, np.array(list(range(150)))/100)
plt.figure(2)
plotSivDistBandsSea(1, 0.1, 2.4, .57, np.array(list(range(150)))/100)