...
class Sivers_Hadron(Hadron):
    ...
    def sivers(self, had, kins, m1, Nu, au, bu, Nubar, Nd, ad, bd, Ndbar, NS, aS, bS, NSbar):
        ...
        + self.NNanti(Nubar,x) * self.eubar**2 * self.pdf(-2, x, QQ) * self.ff(self.ffDict[ii],-2, z, QQ)
        + self.NN(x, Nd, ad, bd) * self.ed**2 * self.pdf(1, x, QQ) * self.ff(self.ffDict[ii],1, z, QQ)
        + self.NNanti(Ndbar,x) * self.edbar**2 * self.pdf(-1, x, QQ) * self.ff(self.ffDict[ii],-1, z, QQ) 
        + self.NN(x, NS, aS, bS) * self.es**2 * self.pdf(3, x, QQ) * self.ff(self.ffDict[ii],3, z, QQ)
        + self.NNanti(NSbar,x) * self.esbar**2 * self.pdf(-3, x, QQ) * self.ff(self.ffDict[ii],-3, z, QQ)
...
class Sivers_DY(DY_Hadron):
    ...
    def sivers(self, kins, m1, Nu, au, bu, Nubar, Nd, ad, bd, Ndbar, NS, aS, bS, NSbar):
        x1 = kins[:, 0]
        x2 = kins[:, 1]
        qT = kins[:, 2]
        QQ = kins[:, 3]
        b0 = self.B0(qT, m1)
        temp_top = self.NN(x1, Nu, au, bu) * self.eu**2 * self.pdf_DY(2, x1, QQ) * self.pdf_DY(-2, x2, QQ)
        + self.NN(x2, Nu, au, bu) * self.eu**2 * self.pdf_DY(2, x2, QQ) * self.pdf_DY(-2, x1, QQ)
        + self.NNanti(Nubar,x1) * self.eubar**2 * self.pdf_DY(-2, x1, QQ) * self.pdf_DY(2, x2, QQ)
        + self.NNanti(Nubar,x2) * self.eubar**2 * self.pdf_DY(-2, x2, QQ) * self.pdf_DY(2, x1, QQ)
        + self.NN(x1, Nd, ad, bd) * self.ed**2 * self.pdf_DY(1, x1, QQ) * self.pdf_DY(-1, x2, QQ)
        + self.NN(x2, Nd, ad, bd) * self.ed**2 * self.pdf_DY(1, x2, QQ) * self.pdf_DY(-1, x1, QQ)
        + self.NNanti(Ndbar,x1) * self.edbar**2 * self.pdf_DY(-1, x1, QQ) * self.pdf_DY(1, x2, QQ)
        + self.NNanti(Ndbar,x2) * self.edbar**2 * self.pdf_DY(-1, x2, QQ) * self.pdf_DY(1, x1, QQ)
        + self.NN(x1, NS, aS, bS) * self.es**2 * self.pdf_DY(3, x1, QQ) * self.pdf_DY(-3, x2, QQ)
        + self.NN(x2, NS, aS, bS) * self.es**2 * self.pdf_DY(3, x2, QQ) * self.pdf_DY(-3, x1, QQ)
        + self.NNanti(NSbar,x1) * self.esbar**2 * self.pdf_DY(-3, x1, QQ) * self.pdf_DY(3, x2, QQ)
        + self.NNanti(NSbar,x2) * self.esbar**2 * self.pdf_DY(-3, x2, QQ) * self.pdf_DY(3, x1, QQ)
