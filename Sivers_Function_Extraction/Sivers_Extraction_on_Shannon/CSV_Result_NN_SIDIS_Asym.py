...
Models_folder = '/project/UVA-Spin/Ishara/Sivers/PseudoDataNNFit_300rep_LR_0000110/Models'
folders_array=os.listdir(Models_folder)
#folders_array=natsort.natsorted(folders_array)
OutputFolder='NN_SIDIS_Fit_Results_cudf'
#os.mkdir(OutputFolder)
#numreplicas_SIDIS=len(folders_array)
numreplicas_SIDIS=5
print(numreplicas_SIDIS)

Org_Data_path = '/home/cee9hc/Sivers/NN_SIDIS_DY/PseudoData_with_DSS/PseudoData_Parm2/'
...
comp17_DY_DATA = cudf.read_csv('../Data/COMPASS_p_DY_2017.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')


HERMES2009 = cudf.concat([herm9_DATA])
HERMES2020 = cudf.concat([herm20_DATA])
COMPASS2009 = cudf.concat([comp9_DATA])
COMPASS2015 = cudf.concat([comp15_DATA])


SIDISdf = cudf.concat([HERMES2009,HERMES2020,COMPASS2009,COMPASS2015])

COMPASS_DY2017 = cudf.concat([comp17_DY_DATA])
...
def GenDYKinematicsFromSIDIS(datasetdf):
    ...
    for i in temp_hads:
        for j in temp_deps:
            ...
            #DYdata_dictionary['y']+= list(T_Kins['y'])
    ...
    return cudf.DataFrame(DYdata_dictionary)
...
def Project_DY_Data_All_Models(SIDIStempdf, numReplicas, sign):
    ...
    return cudf.DataFrame(data_dictionary)
