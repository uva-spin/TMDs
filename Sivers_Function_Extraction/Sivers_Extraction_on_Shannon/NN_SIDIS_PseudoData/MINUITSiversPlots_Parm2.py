...
parms_df = pd.read_csv('./PseudoData_Parm2/Parameters.csv')
...
def SiversFuncAntiQ(dataset,flavor,x,QQ,kp,fitresult):
    ...
    if(flavor==-2):
        ...
        aq=fitresult[5]
        bq=fitresult[6]
        tempsiv=2*NNqbar(x,Nq,aq,bq)*hfunc(kp,tempM1)*(np.exp((-kp**2)/(Kp2A**2)))*(1/((np.pi)*(Kp2A**2)))*(xFxQ2(dataset,flavor,x,QQ))
    if(flavor==-1):
        tempM1=fitresult[0]
        Nq=fitresult[10]
        aq=fitresult[11]
...
plt.savefig('PseudoData_Parm2/SiversQ.pdf')
...
plt.legend(loc=4,fontsize=20,handlelength=3)
plt.savefig('PseudoData_Parm2/SiversAntiQ.pdf')
