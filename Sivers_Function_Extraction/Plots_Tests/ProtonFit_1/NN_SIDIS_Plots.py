...
# Data_Folder = './PseudoData_Parm2'
# HERMES09ex = pd.read_csv(str(Data_Folder)+'/'+'HERMES2009_Pseudo.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
# HERMES09ex_df = pd.concat([HERMES09ex])
# HERMES20ex = pd.read_csv(str(Data_Folder)+'/'+'HERMES2020_Pseudo.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
# HERMES20ex_df = pd.concat([HERMES20ex])
# COMPASS09ex = pd.read_csv(str(Data_Folder)+'/'+'COMPASS2009_Pseudo.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
# COMPASS09ex_df = pd.concat([COMPASS09ex])
# COMPASS15ex = pd.read_csv(str(Data_Folder)+'/'+'COMPASS2015_Pseudo.csv').dropna(axis=0, how='all').dropna(axis=1, how='all')
# COMPASS15ex_df = pd.concat([COMPASS15ex])
...
test_pars=  ([ 3.15, 1.32, 3.37, 14.32, 6.44, 1.51, 0.76, -1.7, 1.3, 13.98, -19.99, 5.78, 20, -4.15, 8.02, 19.99, -15.1, 6.92, 19.79 ])
test_errs=  ([  0.005, 0.006, 0.04, 0.29, 0.023, 0.13, 0.4, 0.11, 0.04, 0.15, 0.11, 0.6, 2.6, 0.4, 0.30, 0.31, 6, 2, 5])
