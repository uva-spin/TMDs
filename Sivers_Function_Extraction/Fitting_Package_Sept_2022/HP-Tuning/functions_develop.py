...
class Sivers_DY(DY_Hadron):
    ...
    def sivers(self, kins, m1, Nu, au, bu, Nub, Nd, ad, bd, Ndb, NS, aS, bS, NSb):
        x1 = kins[:, 0]
        x2 = kins[:, 1]
        qT = kins[:, 2]
        QQ = kins[:, 3] 
        ...
        + NNqbar(x1, NSb) * self.esbar**2 * self.pdf_DY(-3, x1, QQ) * self.pdf_DY(3, x2, QQ)
        + NNqbar(x2, NSb) * self.esbar**2 * self.pdf_DY(-3, x2, QQ) * self.pdf_DY(3, x1, QQ)
