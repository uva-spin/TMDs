...
SIDIS_FFs_Data[2]=(FF_COMPASS_PiP_2009,FF_COMPASS_PiM_2009,FF_COMPASS_Pi0_2009,FF_COMPASS_KP_2009,FF_COMPASS_KM_2009)
SIDIS_FFs_Data[3]=(FF_COMPASS_PiP_2015,FF_COMPASS_PiM_2015,FF_COMPASS_Pi0_2015,FF_COMPASS_KP_2015,FF_COMPASS_KM_2015)
...
def SIDIS_xFxQ2_WOdep(datafile,flavor,hadron):
    # tempvals_all=pd.read_csv(datafile)
    tempvals_all=datafile
...
def SIDIS_zFzQ_WOdep(datafile,flavor,hadron):
#    tempvals=pd.read_csv(datafile)
    tempvals_all=datafile
    tempvals=tempvals_all[(tempvals_all["hadron"]==hadron)]
    tempz=tempvals['z']
...
def Asymmetry_for_Hadron(SIDISdatafilename,hadron,dep,**parms):
    ...
    tempNumerator = uCont1 + ubarCont1 +dCont1 + dbarCont1
    tempDenominator = uCont2 + ubarCont2 +dCont2 + dbarCont2
    tempASiv_Hadron = A0(z,phT,m1,kperp2Avg,pperpAvg,eCharg)*(tempNumerator/tempDenominator)
    return tempASiv_Hadron

     
def Asymmetry_for_Hadron_WOdep(SIDISdatafilename, hadron, kins,**parms):
    m1= parms["m1"]
    Nu = parms["Nu"]
    alphau= parms["au"]
    betau = parms["bu"]
    ...
    phT = kins[:, 2]
    QQ = kins[:, 3]
    uCont1= NNq(x,Nu,alphau,betau)*(eU**2)*SIDIS_xFxQ2_WOdep(PDFfile,2,hadron)*SIDIS_zFzQ_WOdep(FFfile,2,hadron)
    ubarCont1= NNqbar(x,Nubar)*(eUbar**2)*SIDIS_xFxQ2_WOdep(PDFfile,-2,hadron)*SIDIS_zFzQ_WOdep(FFfile,-2,hadron)
    dCont1= NNq(x,Nd,alphad,betad)*(eD**2)*SIDIS_xFxQ2_WOdep(PDFfile,1,hadron)*SIDIS_zFzQ_WOdep(FFfile,1,hadron)
    dbarCont1= NNqbar(x,Ndbar)*(eDbar**2)*SIDIS_xFxQ2_WOdep(PDFfile,-1,hadron)*SIDIS_zFzQ_WOdep(FFfile,-1,hadron)
    # sCont1= NNq(x,Ns,alphas,betas)*(eS**2)*SIDIS_xFxQ2(PDFfile,3,hadron,dep)*SIDIS_zFzQ(FFfile,3,hadron,dep)
    # sbarCont1= NNqbar(x,Nsbar)*(eSbar**2)*SIDIS_xFxQ2(PDFfile,-3,hadron,dep)*SIDIS_zFzQ(FFfile,-3,hadron,dep)
    uCont2= (eU**2)*SIDIS_xFxQ2_WOdep(PDFfile,2,hadron)*SIDIS_zFzQ_WOdep(FFfile,2,hadron)
    ubarCont2= (eUbar**2)*SIDIS_xFxQ2_WOdep(PDFfile,-2,hadron)*SIDIS_zFzQ_WOdep(FFfile,-2,hadron)
    dCont2= (eD**2)*SIDIS_xFxQ2_WOdep(PDFfile,1,hadron)*SIDIS_zFzQ_WOdep(FFfile,1,hadron)
    dbarCont2=(eDbar**2)*SIDIS_xFxQ2_WOdep(PDFfile,-1,hadron)*SIDIS_zFzQ_WOdep(FFfile,-1,hadron)
    # sCont2= (eS**2)*SIDIS_xFxQ2(PDFfile,3,hadron,dep)*SIDIS_zFzQ(FFfile,3,hadron,dep)
    # sbarCont2= (eSbar**2)*SIDIS_xFxQ2(PDFfile,-3,hadron,dep)*SIDIS_zFzQ(FFfile,-3,hadron,dep)
    tempNumerator = uCont1 + ubarCont1 +dCont1 + dbarCont1
    tempDenominator = uCont2 + ubarCont2 +dCont2 + dbarCont2
...
def SIDIStotalchi2Minuit(m1,Nu,au,bu,Nub,Nd,ad,bd,Ndb,Ns,aS,bS,Nsb):
    ...
    for i in range(0,datfilesnum):
        ...
        Nd=Nd,ad=ad,bd=bd,Ndb=Ndb,Ns=Ns,aS=aS,bS=bS,Nsb=Ns))
